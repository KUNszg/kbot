    ...
    if (typeof await req.query != "undefined") {
        ...
        if (!emotes.length) {
            ...
            for (let i=0; i<emotes.length; i++) {
                tableData.push({
                    "ID": `<div class="table-contents" style="text-align: center;">${i+1}</div>`,
                    "emote": `<div class="table-contents" style="text-align: center;"><a href="${emotes[i].url}"><span title="${emotes[i].emote}"><img src="${emotes[i].url}" alt="${emotes[i].emote}"></span></a></div>`,
