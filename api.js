...
webhookHandler.on('*', async function (event, repo, data, head) {
  new utils.WSocket('wsl').emit({
    type: 'github',
    data: [{ event: event }, { repo: repo }, { data: data }, { head: head }],
  });
  ...
  if (event === 'push') {
    await kb.query(
      `
            ...
            SET date=?, sha=?
            WHERE type="ping"`,
      [data.head_commit?.timestamp ?? new Date().toISOString().slice(0, 19).replace('T', ' '), data.head_commit.id.slice(0, 7)]
    );
    ...
    if (data.commits.length > 1) {
      kb.say(
        'ksyncbot',
        `[github webhook] ⬆  New push ${data.head_commit.id.slice(0, 7)} with
    ...
    kb.say(
      'ksyncbot',
      `[github webhook] ⬆  New commit ${data.head_commit.id.slice(0, 7)} in
  ...
  if (event === 'create') {
    if (data.ref_type === 'branch') {
      kb.say(
        'ksyncbot',
        `[github webhook] New branch "${data.ref}" has been created in
    ...
    if (data.ref_type === 'tag') {
      kb.say(
        'ksyncbot',
        `[github webhook] New tag "${data.ref}" has been created in
                ksyncbot's repository`
      );
    }
  }
  ...
  if (event === 'delete') {
    if (data.ref_type === 'branch') {
      kb.say(
        'ksyncbot',
        `[github webhook] Branch "${data.ref}" has been deleted in
                ksyncbot's repository`
      );
    }
    ...
    if (data.ref_type === 'tag') {
      kb.say(
        'ksyncbot',
        `[github webhook] Tag "${data.ref}" has been deleted in
                ksyncbot's repository`
      );
    }
  }
  ...
  if (event === 'commit_comment') {
    if (data.action === 'created') {
      const trim = data.comment.body.length > 350 ? data.comment.body.substring(0, 350) : data.comment.body;

      kb.say(
        'ksyncbot',
        `[github webhook] A commit comment has been created
  ...
  if (event === 'star' && data.action === 'created') {
    ...
    kb.say(
      'ksyncbot',
      `[github webhook] ${data.sender.login} just starred the ksyncbot repository for the total
  ...
  if (event === 'repository_vulnerability_alert') {
    if (data.action === 'create') {
      kb.say(
        'ksyncbot',
        `[github vulnerability alert]  monkaS package ${data.alert.affected_package_name} in
    ...
    if (data.action === 'dismiss') {
      kb.say(
        'ksyncbot',
        `[github vulnerability alert] Vulnerability from ${data.alert.affected_package_name}
  ...
  if (event === 'fork') {
    kb.say(
      'ksyncbot',
      `[github webhook] ${data.sender.login} just forked the repo PogChamp !
  ...
  if (event === 'pull_request') {
    if (data.action === 'opened') {
      kb.say(
        'ksyncbot',
        `[github webhook] ⬇ New pull request #${data.number} "${data.pull_request.title}" 📂  opened by
    ...
    if (data.action === 'closed') {
      ...
      kb.say(
        'ksyncbot',
        `[github webhook] ✅ pull request #${data.number} has been closed ${isMerged} by ${data.sender.login}
  ...
  if (event === 'issues') {
    if (data.action === 'opened') {
      kb.say(
        'ksyncbot',
        `[github webhook] ✅New issue created #${data.issue.number} "${data.issue.title}"
    ...
    if (data.action === 'closed') {
      kb.say(
        'ksyncbot',
        `[github webhook] ⛔ issue #${data.issue.number} has been closed by ${data.sender.login}
    ...
    if (data.action === 'deleted') {
      kb.say(
        'ksyncbot',
        `[github webhook] ❌ issue #${data.issue.number} has been deleted by ${data.sender.login}
...
app.get('/connections', async (req, res) => {
  ...
  let html = fs.readFileSync('../kbot-website/html/express_pages/connections.html');
  ...
  if (Boolean(req.query.details) && !Array.isArray(req.query.details)) {
    ...
    for (let i = 0; i < channels.length; i++) {
      ...
      const tableSize = findLoggedChannel
        ? shell
            .execSync(`sudo du --apparent-size --block=M -s /opt/lampp/var/mysql/kbot/logs_${_channel}.ibd`)
            .toString()
            .split('/')[0]
            .replace('M', '')
        : null;
...
app.get('/countdown', async (req, res) => {
  try {
    ...
    let html = fs.readFileSync('../kbot-website/html/express_pages/countdown.html');
...
app.get('/lastfmresolved', async (req, res) => {
  ...
  let html = fs.readFileSync('../kbot-website/html/express_pages/lastfmResolve.html');
...
app.get('/lastfm', async (req, res) => {
  ...
  let html = fs.readFileSync('../kbot-website/html/express_pages/lastfm.html');
...
app.get('/resolved', async (req, res) => {
  ...
  let html = fs.readFileSync('../kbot-website/html/express_pages/spotifyResolve.html');
      	...
      	<html>
      		...
      		<body style="background-color: #1a1a1a">
                <div style="color: lightgray;">
	          		${new Table({ class: 'table-context' }).setHeaders(headers).setData(tableData).render()}
                </div>
			</body>
		</html>
	    `
...
app.get('/genres', (req, res) => {
  ...
  let html = fs.readFileSync('../kbot-website/html/express_pages/genres.html');
...
app.get('/colors', (req, res) => {
  ...
  let html = fs.readFileSync('../kbot-website/html/express_pages/colors.html');
    ...
    const page = new utils.Swapper(html, [
      {
        ...
        query:
          (await kb.query(`SELECT emotesUpdate FROM channels_logger WHERE channel="${req.query.search.toLowerCase()}"`))[0]?.emotesUpdate ??
          new Date(),
