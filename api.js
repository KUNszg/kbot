...
app.get("/resolved", async (req, res) => {
    ...
    const refresh_token = await fetch(`https://id.twitch.tv/oauth2/token?grant_type=refresh_token&client_id=${creds.client_id}&client_secret=${creds.client_secret}&refresh_token=${req.query.code}`, {
        method: "POST",
        url: `https://id.twitch.tv/oauth2/token?grant_type=refresh_token&client_id=${creds.client_id}&client_secret=${creds.client_secret}&refresh_token=${req.query.code}`,
        headers: {
