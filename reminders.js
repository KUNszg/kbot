...
const unfireEd = async () => {
  ...
  if (differenceToSec > 20) {
    ...
    kb.whisper(
      ...
      `You had an unfired dungeon reminder ${unfiredDiff.toFixed(0)}
...
const reminder = async () => {
  ...
  if (differenceToSec <= 15 && !(differenceToSec < 0)) {
    ...
    if (checkChannelStatus[0].status === 'live') {
      ...
      kb.whisper(
        getUsername[0].username,
        `cookie reminder - eat cookie please :) 🍪 (this reminder fired in a channel that is live [${userData[0].channel}])`
      );
      setTimeout(() => {
    ...
    const restrictedChannels = ['forsen', 'nymn', 'zoil', 'koaster', 'cyr', 'nani'];
