...
(async () => {
  ...
  const aliasListing = async () => {
    ...
    aliasList = aliasList.map(i =>
      JSON.parse('{"' + i.split('>')[0] + '": "' + i.split('>')[1] + '"}')
    );
  ...
  const refreshTokenTwitch = async () => {
    try {
      ...
      console.log(
        `Date: ${currDate.toUTCString()} # Error: ${
          err.message
        } # Request from getTokenTwitch()`
      );
      utils.errorLog(
        `Date: ${currDate.toUTCString()} # Error: ${
          err.message
  ...
  const updateAllEmotes = async channelName => {
    try {
      ...
      try {
        const seventvEmotes =
          (await got(`https://7tv.io/v3/users/twitch/${userId[0].userId}`).json()) || {};

        const emoteSet = seventvEmotes.emote_set || {};
        const _emotes = emoteSet.emotes || [];

        if (_emotes.length) {
          const checkForRepeatedEmotesSeventv = async (emote, id, type, timestamp) => {
            if (typeof seventvEmotes.message != 'undefined') {
              return '';
            }
            ...
            if (!updateEmotes) {
              const emoteLink = `https://cdn.7tv.app/emote/${id}/1x.webp`;
          ...
          for (let j = 0; j < emotes.length; j++) {
            ...
            if (emotes[j].type === '7tv') {
              if (!_emotes.some(i => emotes[j].sevenTvId === i.id)) {
                await kb.query(
                  `
          ...
          _emotes.map(i => checkForRepeatedEmotesSeventv(i.name, i.id, '7tv', i.timestamp));
        }
      } catch (err) {}
  ...
  const updateStats = async () => {
    ...
    let userLoggedInCount = await kb.query(`
        SELECT COUNT(*) AS count
        FROM access_token
        WHERE platform="spotify" OR platform="lastfm" AND user IS NOT NULL`);

    let commandExecutionsCount = await kb.query(`
        SELECT COUNT(*) AS count
        FROM executions
  ...
  const updateViewCount = async () => {
    const channelsViewCount = await kb.query(
    ...
    if (_suggestionsCache.length) {
      for (let i = 0; i < _suggestionsCache.length; i++) {
        ...
        if (entryById && entryById.status !== _suggestionsCache[i].status) {
          [entryById.message, _suggestionsCache[i].message] = new utils.ModifyOutput(
            [entryById.message, _suggestionsCache[i].message],
            200
          ).trimmer();
          ...
          kb.whisper(
            entryById.username,
            `[suggestion update] status of your suggestion
