...
refreshTokenTwitch();
setInterval(()=>{ refreshTokenTwitch() }, 10800000); // every 3h
...
const updateSuggestionsCache = async () => {
    const suggestions = await utils.query('SELECT * FROM suggestions');
    ...
    if (_suggestionsCache.length) {
        for (let i = 0; i < _suggestionsCache.length; i++) { 
            ...
            if (entryById && entryById.status != _suggestionsCache[i].status) {  
                const auth = utils.genString(15);
                const ISODate = new Date().toISOString().slice(0, 19).replace('T', ' ');

                await utils.query("INSERT INTO auth VALUES (?, ?, ?)", ["suggestions", auth, ISODate]);

                [entryById.message, _suggestionsCache[i].message] = new ModifyOutput([entryById.message, _suggestionsCache[i].message], 200).trimmer()

                await got(`https://kunszg.com/api/suggestions?auth=${auth}&old=${JSON.stringify(_suggestionsCache[i])}&new=${JSON.stringify(entryById)}`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/x-www-form-urlencoded"
                    },
...
setInterval(checkSuggestionsChange, 15000);
