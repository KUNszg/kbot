...
const supibotStatusUpdate = async () => {
  try {
    ...
    await got(`https://supinic.com/api/bot-program/bot/active?auth_user=${token[0].user}&auth_key=${token[0].access_token}`, {
      method: 'PUT',
    }).json();
  } catch (err) {
...
const updateViewCount = async () => {
  const channelsViewCount = await kb.query('SELECT viewerCount FROM channels WHERE viewerCount > 0');
