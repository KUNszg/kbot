...
const checkLiveStatus = () => new Promise((resolve) => {
    (async () => {
        const getChannels = await custom.query(`SELECT * FROM channels`);
        const parseForUrl = getChannels.map(i => `&user_login=${i.channel}`)

        const getAccessToken = await custom.query(`SELECT * FROM access_token`);

        const getChannelData = got('https://api.twitch.tv/helix/streams' + parseForUrl.slice(0, 99).join('').replace('&', '?'), {
            method: "GET",
            headers: {
...
const sendChannelStatus = async () => {
    const sendQuery = async (status, name) => {
        await custom.query(`
            UPDATE channels
            SET status=?
            WHERE channel=? AND status="offline"`,
