		...
		invocation: async (channel, user, args) => {
			try {
				...
				if (cookieApi.seconds_left<cookieApi.interval_unformatted-10 || cookieApi.seconds_left === 0) {
					...
					await custom.doQuery(`UPDATE cookie_reminders SET cookie_count="${countCookie[0].cookie_count + 1}" 
						WHERE username="${user['username']}"`)
					const now = new Date();
					await custom.doQuery(`UPDATE cookie_reminders 
	...
	
	{
		...
		invocation: async (channel, user, message, args) => {
			try{
				...
				const checkUsername = await custom.doQuery(`
					SELECT username 
					FROM ed 
					WHERE username="${user['username']}"`);
				...
				const update = await custom.doQuery(`
					UPDATE ed_reminders 
					SET
...
bot.kb.on('chat', async function(channels, user, message) {
...
bot.kb.on('chat', function(channel, user, message) {
	if (channel === '#haxk' && message === "!xd") {
		...
		bot.kb.say('supinic', ' bless u peepoSadDank')
		return;
	} else {
		return; 
	}
