...
const dankeval = [
    {
        ...
        invocation: async (channel, user) => {
            try {
                ...
                const getAlias = await custom.query(`
                    SELECT *
                    FROM user_list
                    WHERE userId=?`,
                    [user['user-id']]);
                ...
                const cookieModule = await custom.query(`
                    SELECT reminders
                    FROM cookieModule
                    WHERE type="cookie"`);
                ...
                const query = await custom.query(`
                    SELECT *
                ...
                custom.errorLog(err);
                kb.whisper(user['username'],  `the cookie API is down, don't panic monkaS / 🐴 `);
                return '';
            }
        }
    },

    {
        ...
        invocation: async (channel, user) => {
            try{
                ...
                const getUserAlias = await custom.query(`
                    SELECT *
                    FROM user_list
                    WHERE userId=?`,
                    [user['user-id']]);
                ...
                const time = now.addMinutes(timeDiff.toFixed(0)).toISOString().slice(0, 19).replace('T', ' ');
                ...
                if (getEdData.next_entry.toFixed(0) > (Date.now(new Date())/1000) &&
                    (getEdData.next_entry.toFixed(0) - (Date.now(new Date())/1000))<3565) {
                        kb.whisper(user['username'], `Your dungeon entry is still on cooldown
                            (${custom.humanizeDuration(getEdData.next_entry.toFixed(0) - (Date.now(new Date())/1000))})
                            to force-set your reminder use "kb ed force".`)
                        return '';
                }

                kb.whisper(user['username'], `I will remind you to enter the dungeon in ${custom.humanizeDuration(timeDiff)} :)`);
...

    {
        ...
        invocation: async (channel, user, message) => {
            try {
                ...
                const getPSData = await custom.query(`
                    SELECT *
                    FROM playsounds
                    WHERE name=?`,
                    [msg[0]]);
                ...
                const now = new Date();
                const time = now.addSeconds(getPSData[0].cooldown/1000).toISOString().slice(0, 19).replace('T', ' ');
