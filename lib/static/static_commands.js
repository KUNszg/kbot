...
setInterval(async () => {
    ...
    kb.say('kunszg', randomEmote[0].emote);
}, 9000000);
...
kb.on("notice", (channel, msgid, message) => {
    if (msgid === "msg_banned") {
        kb.say('kunszg', `@kunszg channel ${channel} has permanently banned the bot :z`);
        return;
    }
    if (msgid === "msg_censored_broadcaster") {
        kb.say(channel, `this message wasn't posted due to conflicts with the channel's moderation settings 😩`);
        return;
    }
    if (msgid === "msg_channel_suspended") {
        kb.say('kunszg', `@kunszg channel ${channel} has been suspended FeelsBadMan`);
        return;
    }
});

kb.on("raw_message", (messageCloned, message) => {
    console.log(message);
