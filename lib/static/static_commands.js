...
kb.on('chat', function(channel, user, message) {
    if (channel === '#haxk' && message.startsWith('!h ')) {
        let findUser = require('./chatters.js').chatters;

        findUser = findUser.filter(i => i[channel.replace('#', '')]);

        const userList = findUser[0][channel.replace('#', '')].filter(i => i.includes('haxk'));
        if (!userList.length) {
            kb.say('haxk', 'ppL');
        } else {
            kb.say('haxk', 'monkaS');
