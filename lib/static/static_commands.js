...
setInterval(async() => {
        const channelList = await custom.doQuery(`
            SELECT *
            FROM channels
            WHERE channel="kunszg"
            `);

        if (channelList[0].status === "live") {
            kb.say('kunszg', 'Song requests are available, use "kb q <song name>" to queue it at the end of the list pepeJAM');
        }
}, 900000);