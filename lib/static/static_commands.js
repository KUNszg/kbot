...
const dankeval = [
...

	{
		...
		invocation: async (channel, user, args) => {
			try {
				...
				if (cookieApi.seconds_left < cookieApi.interval_unformatted-60 || cookieApi.seconds_left === 0) {
		...
		invocation: async (channel, user, message, args) => {
			try{
				...
				if (getEdData.next_entry.toFixed(0) > (Date.now(new Date())/1000) && 
					(getEdData.next_entry.toFixed(0) - (Date.now(new Date())/1000))<3540) {
	...
	/*
	* TODO: 
	* 	- check if playsound is already on global cooldown 
	*	- apply global cooldown to playsound when someone uses "kb rp"
	{
		name: '$ps',
		aliases: null,
		invocation: async (channel, user, message, args) => {
			try {
				...
				const getPSData = await custom.doQuery(`
					...
					WHERE name="${msg[0]}"
					`);
				if (getPSData.length === 0) {
					return '';
				} 
				if (getPSData[0].last_executed === 'null' || (Date.parse(getPSData[0].last_executed) - Date.parse(new Date()))/1000 > 2) {
					return '';
				}
				Date.prototype.addMinutes = function(minutes) {
					var copiedDate = new Date(this.getTime());
				...
				await custom.doQuery(`
					UPDATE playsounds 
					SET last_executed="${now.addMinutes(getPSData[0].cooldown).toISOString().slice(0, 19).replace('T', ' ')}" 
					WHERE name="${msg[0]}"
					`);
