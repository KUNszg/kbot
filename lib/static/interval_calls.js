...
const getTokenSpotify = async () => {
    const userList = await custom.doQuery(`
        SELECT *
        FROM access_token
        WHERE platform="spotify" AND lastRenew < now() - interval 1 hour
        ORDER BY lastRenew
        ASC
        LIMIT 1
        `);
    ...
    const token2 = await got(`https://accounts.spotify.com/api/token?grant_type=refresh_token&refresh_token=AQCuZFik9GUHj7EMRbFGR8g37d7qyq54sqIuNcDn4Zm7FW_ufwkN9S7WwgCPhKNb8Zt7g3-9yQzNisnRwUueN80w_4H9K3Tyt28dITY3_U7kQOC_ox8j1Y9vmP8dqo2ZZkw&client_secret=${creds.client_secret_spotify}&client_id=${creds.client_id_spotify}`, {
            method: "POST",
            headers: {
                "Content-Type": "application/x-www-form-urlencoded"
            },
    ...
    const refreshToken = await custom.doQuery(`
        SELECT *
        FROM access_token
        WHERE platform="spotify" AND user="${userList[0].user}"
        `);
    ...
    try {
        ...
        await custom.doQuery(`
            ...
            WHERE platform="spotify" AND user="${userList[0].user}" AND user!="138403101"
            `);
    } catch (err) {
...
setInterval(()=>{ getTokenSpotify() }, 3000); // every 3s
