...
async function updateCommits() {
	...
	const commitCount = "https://api.github.com/repos/KUNszg/kbot/commits?per_page=100&page=11";
...
setInterval(() => {
    ...
    const expires = new Date(partyData[0].expires);
    const diff = (new Date() - expires)/1000
    console.log(diff)
    if (diff<=15 && !diff<0) {	
		await custom.doQuery(`
			DELETE FROM party WHERE ORDER BY expires ASC LIMIT 1;
			`);
    }
}, 3000);