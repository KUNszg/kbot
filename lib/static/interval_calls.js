...
const getTokenSpotify = async () => {
    try {
        ...
        console.log(`Date: ${currDate.toUTCString()} # Error: ${err.message} # Request URL: ${(err.response?.requestUrl ?? true) ? '[no request url]' : err.response.requestUrl}`);
        custom.errorLog(`Date: ${currDate.toUTCString()} # Error: ${err.message} # Request URL: ${(err.response?.requestUrl ?? true) ? '[no request url]' : err.response.requestUrl}`);
    }
