...
async function getToken() {
	...
	const token = (await fetch(`https://id.twitch.tv/oauth2/token?client_secret=
		${creds.client_secret}&grant_type=refresh_token&refresh_token=${refreshToken[0].refresh_token}`, {
			method: "POST",
...
async function updateCommits() {
	...
	const countCommits = commit.length + 800
...
async function updatePlaysounds() {
...
// update every 1h
setInterval(()=>{updatePlaysounds()}, 3600000);