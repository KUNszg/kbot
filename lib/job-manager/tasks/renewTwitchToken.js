...
const renewTwitchToken = async (kb, config) => {
  ...
  const token = await got(
    `https://id.twitch.tv/oauth2/token?client_secret=${config.client_secret}&grant_type=refresh_token&refresh_token=${refreshToken}`,
    {
      method: 'POST',
      headers: {
