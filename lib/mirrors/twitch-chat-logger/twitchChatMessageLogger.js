...
(async () => {
  ...
  startHeartbeat(kb, 'twitch-chat-message-logger');
    ...
    const processCache = async () => {
      ...
      try {
        ...
        if (allUserData.length > 0) {
          ...
          await kb.sqlClient.query(
            `INSERT IGNORE INTO user_list (username, userId, lastSeen, firstSeen,  color, added)
             VALUES ${userValues}`,
            allUserData.flat()
          );

          const updatePromises = allUserData.map(
            ([username, userId, lastSeen, firstSeen, color]) =>
              kb.sqlClient.query(
                `UPDATE user_list SET lastSeen=?, color = ?  WHERE username=?`,
                [lastSeen, color, username]
              )
          );
