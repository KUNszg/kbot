...
const kb = require('../commandManager_old.js').kb;
  ...
  invocation: async (channel, user, message) => {
    try {
          ...
          if (checkIfRenewed.length) {
            try {
              const tokenSpotify = await got(
                `https://accounts.spotify.com/api/token?grant_type=refresh_token&refresh_token=${await refresh(
                  'refresh_token',
                  username
                )}&client_secret=${creds.client_secret_spotify}&client_id=${
                  creds.client_id_spotify
                }`,
                {
                  method: 'POST',
                  headers: {
        ...
        if (checkIfRenewed.length) {
          try {
            ...
            if (
              ...
              kb.whisper(
                user.username,
                `Your Spotify token has been revoked by Spotify, you will have to register for this command again to use it. https://kunszg.com/connections`
              );
              return '';
            }
            console.log(err);
        ...
        if (msg[0].startsWith('@')) {
          ...
          const response = `${
            ...
            '$1\u{E0000}'
                    ...
                    [${minutes(song.progress_ms)}:${seconds(song.progress_ms)}/${minutes(
            song.item.duration_ms
          )}:${seconds(song.item.duration_ms)}] `;

          try {
            const youtube = await utils.youtube(
              `${song.item.name} by ${song.item.artists[0].name}`
            );
            ...
            if (msg.filter(i => i === '-t').length) {
              return (
                response +
                `https://youtu.be/${youtube.id}?t=${Math.round(song.progress_ms / 1000)}`
              );
            }
      ...
      if (msg[0] === 'play' || msg[0] === 'queue' || msg[0] === 'p' || msg[0] === 'q') {
        try {
          ...
          const search = await apiCall(
            `https://api.spotify.com/v1/search?q=${msg.splice(1).join(' ')}&type=track`,
            user.username,
            'GET'
          );
          ...
          if (msg[0] === 'q' || msg[0] === 'queue') {
            await apiCall(
              `https://api.spotify.com/v1/me/player/queue?uri=${search.tracks.items[0].uri}`,
              user.username,
              'POST'
            );
        ...
        if (findUser) {
          ...
          if (msg.find(i => i.match(/\bartist[s]?\b/i))) {
            let topArtistData = await apiCall(
              `https://api.spotify.com/v1/me/top/artists?time_range=${timeRange.toLowerCase()}_term`,
              _user
            );
          ...
          let topTracksData = await apiCall(
            `https://api.spotify.com/v1/me/top/tracks?time_range=${timeRange.toLowerCase()}_term`,
            _user
          );

          const songs = assignNumbers(
            new utils.ModifyOutput(topTracksData.items.slice(0, 4).map(i => i.name)).trimmer()
          );
      ...
      if (msg[0] === 'search') {
        ...
        const search = await apiCall(
          `https://api.spotify.com/v1/search?type=${type}&q=${query}&limit=1&offset=0`,
          user.username,
          'GET'
        );
      ...
      if (msg[0] === 'yoink') {
        ...
        if (checkCache) {
          try {
            ...
            if (getFlag('q')) {
              await apiCall(
                `https://api.spotify.com/v1/me/player/queue?uri=${checkCache.uri}`,
                user.username,
                'POST'
              );
        ...
        try {
          ...
          if (getFlag('t')) {
            ...
            timestampMessage = ` at timestamp [${minutes(yoinkedSong.progress_ms)}:${seconds(
              yoinkedSong.progress_ms
            )}/
                        ${minutes(yoinkedSong.item.duration_ms)}:${seconds(
              yoinkedSong.item.duration_ms
            )}]`;
          }

          if (getFlag('q')) {
            await apiCall(
              `https://api.spotify.com/v1/me/player/queue?uri=${yoinkedSong.item.uri}`,
              user.username,
              'POST'
            );
          ...
          const [playing, artist] = new utils.ModifyOutput([
            yoinkedSong.item.name,
            yoinkedSong.item.artists[0].name,
          ]).trimmer();
      ...
      if (msg[0] === 'volume' || msg[0] === 'vol') {
        ...
        await apiCall(
          `https://api.spotify.com/v1/me/player/volume?volume_percent=${this.msg}`,
          user.username,
          'PUT'
        );
