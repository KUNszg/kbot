  ...
  invocation: async (channel, user, message, platform) => {
    try {
      ...
      if (checkIfDecimal.test(msg[0])) {
        ...
        // find genre by key
        const getKeyByName = Object.keys(genres).filter(
          i => i.toLowerCase() === msg.join(' ').toLowerCase()
        );
        ...
        genres = Object.entries(genres).filter(
          i => i[0].toLowerCase() === msg.join(' ').toLowerCase()
        )[0][1];
      }
      ...
      const track = await got(
        `https://api.musixmatch.com/ws/1.1/track.search?format=json&apikey=${creds.randomTrack}&s_track_rating=desc&f_music_genre_id=${genres}&page_size=1`
      ).json();
      if (track.message.header.available > 0) {
        const pages = Math.ceil(track.message.header.available / 100);
        const page = Math.floor(Math.random() * pages + 1);
        const randomTrack = await got(
          `https://api.musixmatch.com/ws/1.1/track.search?format=json&apikey=${creds.randomTrack}&s_track_rating=desc&f_music_genre_id=${genres}&page_size=100&page=${page}`
        ).json();
      ...
      const youtube = await utils.youtube(
        `${songData.track.track_name} by ${songData.track.artist_name}`
      );
