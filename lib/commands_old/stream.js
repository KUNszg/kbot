...
module.exports = {
  ...
  invocation: async (channel, user, message) => {
    try {
      ...
      const msg =
        message.split(' ')[1].toLowerCase() === 'title' ||
        message.split(' ')[1].toLowerCase() === 'game'
      ...
      if (msg[0] === 'title') {
        await got(
          `https://api.twitch.tv/helix/channels?broadcaster_id=${
            owner[0].userId
          }&title=${encodeURI(msg.splice(1).join(' '))}`,
          {
            method: 'PATCH',
            headers: {
      ...
      if (msg[0] === 'game') {
        const gameId = await got(
          `https://api.twitch.tv/helix/games?name=${encodeURI(msg.splice(1).join(' '))}`,
          {
            method: 'GET',
            headers: {
              Authorization: `Bearer ${getAccessToken[0].access_token}`,
        ...
        await got(
          `https://api.twitch.tv/helix/channels?broadcaster_id=${owner[0].userId}&game_id=${gameId.data[0].id}`,
          {
            method: 'PATCH',
            headers: {
              Authorization: `Bearer ${getAccessToken[0].access_token}`,
