...
module.exports = {
  ...
  invocation: async (channel, user, message, platform) => {
    try {
      ...
      if (msg[0] === 'backup') {
        const lastBackup = await kb.query(`SELECT * FROM backup_history`);

        let backupStart = lastBackup.filter(i => i.type === 'start');
        backupStart =
          (Date.now() - new Date(backupStart[backupStart.length - 1].date).getTime()) / 1000;

        let backupEnd = lastBackup.filter(i => i.type === 'end');
        backupEnd =
          (Date.now() - new Date(backupEnd[backupEnd.length - 1].date).getTime()) / 1000;

        return `${
          user.username
        }, last backup of the database and bot files started ${utils.humanizeDuration(
          backupStart
        )} ago and finished ${utils.humanizeDuration(backupEnd)} ago`;
      }
      ...
      if (msg[0] === 'flag') {
        ...
        if (!findUser.length) {
          findUser = await got(`${api}${msg[1]}`).json();
        ...
        await utils.query(
          `
                    ...
                    VALUES (?, ? ,?)`,
          [findUser.userId, findUser.username, msg[2]]
        );
      ...
      if (msg[0] + ' ' + msg[1] === 'ban #all') {
        let activeChatters = await got(
          `https://tmi.twitch.tv/group/user/${channel.replace('#', '')}/chatters`
        ).json();
        ...
        for (let i = 0; i < activeChatters.length; i++) {
          kb.say(
            channel,
            `/ban ${activeChatters[i].toLowerCase() === 'supibot' ? '' : activeChatters[i]}`
          );
        }
      ...
      if (msg[0] + ' ' + msg[1] === 'unban #all') {
        ...
        for (let i = 0; i < activeChatters.length; i++) {
          kb.say(
            channel,
            `/unban ${activeChatters[i].toLowerCase() === 'supibot' ? '' : activeChatters[i]}`
          );
        }
