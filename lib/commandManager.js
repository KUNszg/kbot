...
(async () => {
  ...
  const processTwitchChatMessage = async (channel, user, message, self) => {
    ...
    if (!invokedCommandResult.messageContext) {
      invokedCommandResult.messageContext = _.bind(kb.tmiClient.sender.say, kb.tmiClient.sender)
    }
    ...
    timeout: (channel, username, message, duration, msg) => {
      if (channel === '#supinic') {
        if (duration === '1') {
          ...
          kb.tmiClient.sender.say(channel, `${username} has been permanently banned MODS Clap`);
        } else {
          kb.tmiClient.sender.say(
            channel,
