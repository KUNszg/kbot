...
(async () => {
  ...
  async function handler(channel, user, message, self, event, messageState) {
    ...
    if (event === 'discord') {
      ...
      let result = await commands[input[1].toLowerCase()].invocation(channel, user, message, event, messageState);
    ...
    if (event === 'chat') {
      if (channel === '#pajlada') {
        if (message === 'pajaS 🚨 ALERT' && user['user-id'] === '82008718') {
          kb.action(channel, utils.random(['KKurwa 🚨 NAURA', 'Clue TeaTime 🚨 TMI MELTDOWN']));
          return;
        }
    ...
    if (event === 'chat') {
      ...
      if (input[1].match(/\brl|rq|ls|fl\b/g) && typeof input[2] != 'undefined') {
        ...
        const userDetails = await Promise.all([utils.Get.user().byUsername(input[2]), utils.Get.user().byUsername(inputFindUser)]);
  ...
  discord.on('message', message => {
    ...
    if (message.guild.id === '747561445998329957') {
      ...
      const input = msg.replace(Alias.getRegex(), Alias.getReplacement()).replace(regex.invisChar, '').split(' ');
...
discord.on('ready', () => {
  setTimeout(async () => {
    ...
    setInterval(async () => {
      const post = await utils.query(`SELECT MIN(ID), data FROM livethreads WHERE resolved = "N"`);
      ...
      await utils.query(`UPDATE livethreads SET resolved = "Y" WHERE ID = ?`, [post[0]['MIN(ID)']]);

      for (let i = 0; i < liveThreadChannels.length; i++) {
        const channel = discord.channels.cache.find(channel => channel.id === liveThreadChannels[i].channelID);

        if (liveThreadChannels[i].threadID) {
          const thread = channel.threads.cache.find(thread => thread.id === liveThreadChannels[i].threadID);
    ...
    const savePost = async data => {
      if (!lastPost.filter(cache => cache.id === data.id).length) {
        if (!data.body.includes('reddit.com')) {
          const repeats = await kb.query('SELECT * FROM livethreads WHERE DATA LIKE "%?%"', [data.body.match(regex.url)]);
