...
kb.on("chat", async (channel, user, message, self) => {
	...
	const input = message
		...
		.replace(/\bkb 4head\b/i, 'kb 4head')
		.replace(/\bkb chn\b/i, 'kb channels')
		.replace(/\bkb location\b/i, 'kb locate')
		.replace(/\bkb yomama\b/i, 'kb joemama')
		.replace(/\bkb git\b/i, 'kb github')
		.replace(/\bkb gg\b/i, 'kb google')
		.replace(/\bkb dailysurah\b/i, 'kb surah')
		.replace(/\bkb daily\b/i, 'kb surah')
		.replace(/\bkb kiss\b/i, 'kb hug')
		.replace(/\bkb bp\b/i, 'kb banphrase')
		.split(' ')
	//	.replace('ct', 'chat')
