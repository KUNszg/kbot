...
kb.on("chat", async (channel, user, message, self) => {
    const aliasList = request('../data/aliases.json');
    const findAlias = (input) => {
        return new RegExp(`\\b${input}\\b`, "i")
    }

    const alias = aliasList.data.filter(i => i === message.split(' ')[1])
    console.log(alias)
    ...
    const input = message
        ...
        .replace(/\bquran\b/i, 'surah')
        .replace(/\ballah\b/i, 'surah')
        ...
        .replace(/\bremoved\b/i, 'emotes removed')
        .split(' ')
