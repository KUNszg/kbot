...
(async () => {
    ...
    async function eventEmitter(channel, user, message, self, event, messageState) {
        ...
        if (event === "chat" && utils.strictChannels(channel, true)) {
            if ([...talkedRecently.values()].find(i => i === channel) && (await utils.checkPermissions(user.username)) < 1) { return; }

            talkedRecently.add(channel);

            this.timeout = 2500;
            if (_channels.find(i => i.channel === channel.replace("#", "") && i.status === "live")) {
                this.timeout = 8000;
            }
    ...
    */
    ...
    kb.on("action", async (channel, user, message, self) => {
        await eventEmitter(channel, user, message, self, "chat");
    });
    ...
    kb.on("whisper", async (username, user, message, self) => {
        await eventEmitter(username, user, message, self, "whisper");
    });
    ...
    */
    ...
    discord.on('message', message => {
        ...
        eventEmitter(this.channel, this.user, this.message, this.self, "discord", message);
    });
})();
...
(async () => {
    ...
    for (let i = 0; i < results.length; i++) {
        ...
        if (!compareCommands.length) {
            await utils.query(`
                INSERT INTO commands_deleted (command, date)
                VALUES (?, CURRENT_TIMESTAMP)`,
                [results[i].command]);
