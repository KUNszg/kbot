...
kb.on("chat", async (channel, user, message, self) => {
    ...
    let alias = aliasList.data.filter(i => i[param])[0];

    if (typeof alias === "undefined") {
        alias = '';
    }

    const getRegex = convertToRegexp(Object.keys(alias));
    const getReplacement = Object.values(alias)[0];

    console.log(getRegex, getReplacement)
