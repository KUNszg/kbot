...
kb.on("chat", async (channel, user, message, self) => {
    ...
    if (!checkUsersPinged.length) {
        ...
        const diff = ((Date.now() - Date.parse(getUptime[0].date))/1000).toFixed(2); // get time difference between when bot was launched and current time in seconds
        ...
        kb.say(channel, `${user['username']}, bot is still loading ⌛ ${Math.abs(diff - getChannelId[0].ID * 3)} more seconds for this channel, please wait :)`);
        return;
    }
