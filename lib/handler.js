...
setTimeout(() => {
    ...
    const sendToThread = (data) => {
        if (!lastPost.filter(id => id === data.id).length) {
            if (!data.body.includes("[A new live thread has been posted.]")) {
                thread.send(data.body.replace("https://twitter.com", "https://fxtwitter.com"));
                lastPost.push(data.id);
            }
        }
    }

    r.getLivethread('18hnzysb1elcs').stream.on('update', sendToThread);
}, 5000);
