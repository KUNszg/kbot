...
setTimeout(async() => {
    ...
    const sendToThread = async(data) => {
        if (!lastPost.filter(cache => cache.id === data.id).length) {
            if (!data.body.includes("reddit.com")) {
                kb.query("INSERT INTO livethreads (data, date) VALUES (?, ?)", [JSON.stringify(data), "CURRENT_TIMESTAMP"]);

                if (cooldown.has("await")) {
                    return;
                }
                cooldown.add("await");
                setTimeout(() => {
                    cooldown.delete("await");
                }, 300000);

                const post = await kb.query(`SELECT MIN(ID) FROM livethreads WHERE resolved != "N"`);
                const postData = JSON.parse(post[0].data);

                thread.send(postData.body);
                lastPost.push(postData);

                await kb.query(`UPDATE livethreads SET resolved = "Y" WHERE ID = ?`, [post[0].ID]);
            }
        }
    }

    r.getLivethread('18hnzysb1elcs').stream.on('update', await sendToThread);
}, 5000);
