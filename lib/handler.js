...
kb.on("chat", async (channel, user, message, self) => {
	...
	checkUsersPinged = checkUsersPinged.filter(i => i[channel]);

	if (checkUsersPinged.length === 0) {
		kb.say(channel, `${user['username']}, ping-prevention module is still loading, please wait :)`);
	}

	checkUsersPinged = result.split(' ').filter(i => checkUsersPinged.filter(j => i.toLowerCase().includes(j)));
	
	if (checkUsersPinged[0][channel].length>6) {
		kb.say(channel, `${user['username']}, too many users pinged by the result of this command.`);
		return;
	}
