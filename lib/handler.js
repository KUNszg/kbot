...
setTimeout(async() => {
    ...
    setInterval(async () => {
        ...
        let response = await twitterGetUrl(postData.body.match(regex.url)[0]);

        if (response.found) {
            const newUrl = postData.body.match(regex.url)[0].replace("https://twitter.com", "https://fxtwitter.com");
            thread.send(postData.body.replace(postData.body.match(regex.url)[0], newUrl));
        }
        else {
