...
kb.on("chat", async (channel, user, message, self) => {
    ...
    const parseResult = (typeof result === "undefined" || !result) ? '' : result.replace(regex.invisChar, '');
    ...
    try {
        ...
        if (test.banned) {
            if (utils.strictChannels(channel) && utils.ignore(channel)) {
                const response = `${user['username']}, the result is banphrased, I whispered it to you tho cmonBruh`;
                if (await utils.banphrasePass(response, channel.replace('#', ''))) {
                    kb.say(channel, "response contains a banphrased username");
                    kb.whisper(user['username'], result);
                    return;
                }
                kb.say(channel, response);
                kb.whisper(user['username'], result);
                commands.length = 0;
                return;
            }
