        ...
        if (tweetURL) {
            ...
            if (tweetURL.includes("twitter.com") && urlParts[4] === "status") {
                ...
                if (tweetData.errors) {
                    ...
                    if (tweetData.data[0].attachments) {
                        result = tweetData.data[0].attachments.media_keys.length + " picture(s) detected in this tweet\n\n"
                    }

                    if (tweetData["includes"]) {
                        const media = tweetData["includes"].media[0];
                        if (media.type === "video") {
                            result = "video length - " + media.duration_ms/1000 +
                                "s\nview count - " + media.public_metric.view_count + "\n\n" +
