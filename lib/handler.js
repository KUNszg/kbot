...
setTimeout(async() => {
    ...
    setInterval(async () => {
        ...
        const tweetURL = postData.body.match(regex.url)[0];
        ...
        if (tweetURL.includes("twitter.com")) {
            ...
            const token = await got(url + tweetID + args, {
                method: "GET",
                headers: {
            ...
            if (token.data[0].attachments || token.data[0]["includes"]) {
                thread.send(fxTwitter);
            }
            else {
