...
wssLocal.on('connection', function connection(ws) {
  ws.on('message', function incoming(message) {
    ...
    const auth = Math.random().toString(20).substr(2, 15);
...
const refresh = async () => {
  const _usersTotal = await utils.query('SELECT COUNT(userId) as count FROM user_list');
  const _commandExecs = await utils.query('SELECT COUNT(ID) as count FROM executions');

  return {
    body: [
      { type: 'usersTotal', data: _usersTotal[0].count },
      { type: 'commandExecs', data: _commandExecs[0].count },
    ],
  };
};
...
wssPublic.on('connection', async function connection(ws, req) {
  ...
  ws.on('message', async message => {
    ...
    const auth = fs
      .readFileSync(`../../data/temp_websockets_auth${message.authId}.txt`)
      .toString();
