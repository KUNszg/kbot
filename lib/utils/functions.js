...
exports.doQuery = (query) => new Promise((resolve, reject) => {
    database.execute(mysql.format(query), async(err, results, fields) => {
        if (err) {
            console.log(err)
...
exports.banphrasePass = (output, channel) => new Promise((resolve, reject) => {
    ...
    if (channelParsed === "forsen") {
        ...
        const banphrasePass = (
            fetch('https://pajlada.pajbot.com/api/v1/banphrases/test', {
                method: "POST",
                url: "https://pajlada.pajbot.com/api/v1/banphrases/test",
                body: "message=" + output,
                headers: {
                    "Content-Type": "application/x-www-form-urlencoded"
                },
    ...
    // bp api for #haxk  https://haxk.botfactory.live/commands
...
exports.strictChannels = async (channel) => {
    const channels = [
        '#nymn',
        '#forsen',
        '#vadikus007',
