...
exports.Cooldown = class Cooldown {
    ...
    async cooldownReduction() {
        const cooldown = Number((await utils.query(`
            SELECT cooldown
            FROM commands
            WHERE command=?`,
            [this.command])
...
exports.Tmi = {
    send: async function (channel, message, opts) {
        ...
        const repeatedMessages = () => {
            ...
            if (lastItem.length > 0) {
                ...
                if (lastItem.message.replace("\u{E0000}", "") === data.message && lastItem.channel === data.channel) {
                    lastItem.message.endsWith("\u{E0000}") ? data.message = data.message.replace(/\+$/, "") : data.message += "\u{E0000}";
                }
            }
        ...
        if (this.opts.ban) {
            this.opts.banMessage = this.opts.banMessage ? this.opts.banMessage : "no message";
        ...
        if (this.opts.timeout) {
            this.opts.timeoutReason = this.opts.timeoutReason ? this.opts.timeoutReason : "no message";
            this.opts.timeoutDuration = this.opts.timeoutDuration ? this.opts.timeoutDuration : 1;
