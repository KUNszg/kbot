...
exports.banphrasePass = (output, channel) =>
  new Promise(async (resolve, reject) => {
    ...
    const banphrase = await got(encodeURI(data[0].url), {
      method: 'POST',
      body: 'message=' + encodeURIComponent(output),
      headers: {
...
exports.status = code => {
  switch (String(code)) {
    ...
    case '415':
      return (
        '[client err 415 Unsupported Media Type] - the request entity has a media type which the ' + 'server or resource does not support. '
      );
...
exports.Alias = class Alias {
  constructor(message) {
    this.command = message.replace(regex.invisChar, '').split(' ').splice(1).filter(Boolean)[0];
...
exports.Get = {
  ...
  userData: {
    ...
    bannedMention: function (userstate) {
      if (userstate) {
        return db.query(
          `
                    SELECT *
      ...
      return db.query('SELECT * FROM mention_bans');
    },
  ...
  api() {
    return Object.create(this.apiData);
  },
  apiData: {
    url: async function (command_name) {
      const Alias = new utils.Alias(command_name);
      const input = command_name.replace(Alias.getRegex(), Alias.getReplacement()).replace(regex.invisChar, '');
    ...
    key: async function (command_name) {
      const Alias = new utils.Alias(command_name);
      const input = command_name.replace(Alias.getRegex(), Alias.getReplacement()).replace(regex.invisChar, '');
...
exports.Swapper = class Swapper {
  ...
  template() {
    for (let i = 0; i < this.valueKeys.length; i++) {
      this.html = this.html.replace(this.valueKeys[i], this.value[this.valueKeys[i].replace(/^%{/, '').replace(/}$/, '')]);
    }
    return this.html;
  }
};
...
exports.ModifyOutput = class ModifyOutput {
  ...
  trimmer() {
    ...
    if (!Array.isArray(this.input)) {
      return this.input.length > this.trim ? `${noPing(this.input.substr(0, this.trim))}(...)` : noPing(this.input);
    }

    let result = [];

    for (let i = 0; i < this.input.length; i++) {
      result.push(this.input[i].length > this.trim ? `${noPing(this.input[i].substr(0, this.trim))}(...)` : noPing(this.input[i]));
    }
    return result;
  }
};
