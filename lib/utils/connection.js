...
class SQL extends EventEmitter {
    ...
    execute(query, data) {
        return this.con.execute(query, data);
    }

    unprepare(query) {
        return this.con.unprepare(query);
    }
}
...
class IRC extends SQL {
    ...
    async connect(type) {
        ...
        if (process.platform === "win32") {
            channels = [(await utils.Get.user().owner())[0].username];
        } else {
            if (type.logger) {
                channels = await utils.query("SELECT * FROM channels_logger");
            } else {
        ...
        // Called when any error occurs on the client, including non-fatal
        // errors such as a message that could not be delivered due to an error.
        ...
        this.client.on("CLEARCHAT", (clearchatMessage) => {
            this.emit("clearchat", clearchatMessage);
        });
        ...
        this.client.on("NOTICE", (noticeMessage) => {
            this.emit("notice", noticeMessage);
        });
