...
class AMQP extends SQL {
  constructor() {
    super();
  }

  async rabbitConnect() {
    this.rabbitCon = await amqplib.connect({
      protocol: 'amqp',
      hostname: 'localhost',
  ...
  async createRabbitChannel(queue) {
    ...
    await consumer.consume(queue, msg => {
      this.emit('rabbitConsumer', msg.content.toString());
    });
  }
...
module.exports = { IRC, SQL, AMQP, Redis };
