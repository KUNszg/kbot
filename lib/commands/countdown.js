...
module.exports = {
    ...
    invocation: async (channel, user, message) => {
        try {
            ...
            if (channel === "#forsen") {
                return `${user['username']}, your countdown will end in ${custom.humanizeDuration(seconds)}
                kunszg(dot)com/countdown?seconds=${seconds}&verifcode=${code}`;
            }
            return `${user['username']}, your countdown will end in ${custom.humanizeDuration(seconds)}
            https://kunszg.com/countdown?seconds=${seconds}&verifcode=${code}`;
        } catch (err) {
            custom.errorLog(err)
