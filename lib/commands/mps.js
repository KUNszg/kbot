...
module.exports = {
	name: prefix + 'mps',
	aliases: null,
	description: `messages per minute for current channel -- cooldown 5s`,
	permission: 0,
	cooldown: 5000,
	invocation: async (channel, user, message, args) => {
		try {
			...
			if (!msg[0]) {
				...
				bot.kb.say(channel, `${user['username']}, this channel has currently 
					${cache.length} messages per minute.`);
			}
			return '';
