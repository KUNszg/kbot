...
module.exports = {
	...
	invocation: async (channel, user, message, args) => {
		try {
			...
			if (!msg) {
                if (channel === '#forsen') {
                    kb.whisper(user['username'], `, list of genres with command examples: http://kunszg.xyz/genres`);
                    return `${user['username']}, I whispered you response :)`;
                }
				return `${user['username']}, list of genres with command examples: http://kunszg.xyz/genres`;
			}
            ...
            if (checkIfDecimal.test(msg[0])) {
                ...
                if (typeof getKeyByValue === 'undefined') {
                    return `${user['username']}, specified genre number does not exist, list of genres: http://kunszg.xyz/genres`;
                }
                genres = Number(msg[0]);
            } else {
                // find genre by key
                ...
                if (!getKeyByName.length) {
                    return `${user['username']}, specified genre name does not exist, list of genres: http://kunszg.xyz/genres`;
                }
