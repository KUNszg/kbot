    ...
    invocation: async (channel, user, message, args) => {
        try {
            ...
            switch (msg[0]) {
                ...
                case 'register':
                    ...
                    if (!userData.length) {
                        ...
                        if (channel === "#forsen") {
                            ...
                            a cookie reminder, see https://kunszg (dot) xyz/commands for command options`;
                        }
                        return `${user['username']}, you have been successfully registered for
                        a cookie reminder, see https://kunszg.xyz/commands for command options`;
                    }
                ...
                default:
                    if (channel==="#forsen") {
                        return `${user['username']}, invalid syntax. See kunszg(dot)xyz/commands for command options.`;
                    }
                    return `${user['username']}, invalid syntax. See https://kunszg.xyz/commands for command options.`;
            }
            return '';
        } catch (err) {
            custom.errorLog(err)
