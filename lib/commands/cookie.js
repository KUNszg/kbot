...
module.exports = {
    name: prefix + 'cookie',
    aliases: null,
    description: `ThePositiveBot's cookie reminders, (NOTE: this command is not a minigame, it's just a REMINDER for ThePositiveBot's cookies) usage: https://kunszg.xyz/commands#cookie_opt -- cooldown 8s`,
    description_formatted: `ThePositiveBot's cookie reminders, (NOTE: this is not a minigame, it's just a REMINDER for ThePositiveBot's cookies), usage: https://kunszg (dot) xyz/commands#cookie_opt -- cooldown 8s`,
    permission: 0,
    cooldown: 8000,
    invocation: async (channel, user, message, args) => {
        try {
            ...
            switch (msg[0]) {
                    ...
                    if (resultsRegister[0].user_alias === getAlias[0].ID &&
                        resultsRegister[0].initplatform === 'whisper') {
                            ...
                            await custom.doQuery(`
                                ...
                                return `${user['username']}, you have changed your feedback message to appear in
                                your own channel. Type this command again to undo it.`;
                            }
                ...
                default:
                    if (channel==="#forsen") {
                        return `${user['username']}, invalid syntax. See https://kunszg (dot) xyz/commands#cookie_opt for command options.`;
                    }
