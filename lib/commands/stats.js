...
module.exports = {
	...
	name: prefix + 'stats',
	aliases: null,
	description: `syntax: https://pastebin.com/1T48gpuk -- cooldown 60s`,
    description_formatted: 'syntax: pastebin (dot) com/1T48gpuk -- cooldown 60s',
	permission: 0,
	cooldown: 60000,
	invocation: async (channel, user, message, args) => {
		try {
            ...
            // cooldown between command executions in all chats
			const globalCooldown = new Set();
			if (globalCooldown.has('cd')) {
				return '';
			}
			globalCooldown.add('cd');
			setTimeout(() => {
				globalCooldown.delete('cd');
			}, 10000);
			...
			if (checkIfBanned.length != 0 && (channel === "#nymn" || channel === "#forsen")) {
			...
			const checkChannel = await custom.doQuery(`SHOW TABLES LIKE "logs_${channelParsed}"`)
			...
			if (channel === "#forsen" && (msg[0] != '-lines' && (msg[0] != "-channel" && ((msg[0] != "-bruh" && msg.length != 0) && (!msgCd.has(user['user-id'])))))) {
				msgCd.add(user['user-id']);
				setTimeout(() => {
					msgCd.delete(user['user-id']);
				}, 1800000);
				bot.kb.whisper(user['username'], 'performing a big query, please wait. It may take up to 30 seconds to get results.');
			}

			// if no parameters provided...
			if (msg[0] != '-lines' && ((msg[0] != "-channel" && msg[0] != "-bruh") && msg.length != 0)) {
				...
				if (msg.filter(i => i.startsWith('@')).toString().includes('@')) {
					...
					if (channel === '#nymn' || channel === '#forsen') {
						...
						if (compile[0][0].message.toString().length>50) {
							...
							if (await custom.banphrasePass(modifyOutput(), channel.replace('#', '')).banned === true) {
				...
				if (!msg[1]) {
					...
					if (channel === '#haxk') {
						...
						const channelValueCache = await custom.doQuery(`
							...
							`);
						if (channelValueCache[0].count === 0) {
				...
				// kb stats -bruh [user]
					...
					if (msg[1].toLowerCase() === 'teodorv' || msg[1].toLowerCase() === 'phzeera') {
            ...
            // kb stats
				...
				if (channel === "#forsen") {
					bot.kb.whisper(user['username'], 'this syntax is disabled in this channel for now, you can still use kb stats [message] or kb stats -bruh etc.');
					return '';
				}
