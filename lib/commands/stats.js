...
module.exports = {
	...
	logs in my database | -channel - information about the current channel | -bruh - amount of unpleasant people in chat 
	| [input] - provide a custom message | @[user] - searches for given user -- cooldown 60s`,
	permission: 0,
	cooldown: 60000,
	invocation: async (channel, user, message, args) => {
		try {
			...
			if (channel === "#forsen" && (msg[0] != "-bruh" && msg.length != 0) && (!msgCd.has(user['user-id']))) {
			...
			if (((msg[0] != "-channel" && msg[0] != "-bruh") && msg.length != 0)) {	
				...
				if (msg[1].toLowerCase() === 'apollo' && channel === "#nymn") {
					...
					await custom.doQuery(`
						UPDATE stats
						SET count="${Number(getCount[0].count)+(Math.random()*10).toFixed(0)}"
						WHERE sha="apollo"
						`);
					return `${user['username']}, Apollo said it ${getCount[0].count} times TriSad`
				}
