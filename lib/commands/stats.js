...
module.exports = {
    ...
    invocation: async (channel, user, message, platform) => {
        try {
            ...
            const msg = custom.getParam(message.replace(/[^-?]\bcolor\b/, ' stats -color').replace(/[^-?]\bcolour\b/, ' stats -color'));

            // check for internal banphrases
            const getInternalBans = await custom.query(`SELECT * FROM internal_banphrases`);

            const checkIfBanned = getInternalBans.filter(i => msg.join(' ').includes(i.banphrase));

            if (checkIfBanned.length && custom.strictChannels(channel)) {
            ...
            const checkChannel = await custom.query(
                "SHOW TABLES LIKE ?",
                [`logs_${this.channel}`]);
            ...
            /* kb stats */
            const stats = async() => {
                if (channel === "#forsen" || channel === "#nymn" || channel === "#vadikus007") {
                ...
                const occurence = await  custom.query(`
                    SELECT MAX(ID) as value
                    FROM ??`,
                    [`logs_${this.channel}`])

                // channel lines occurence
                const val = await custom.query(`
                ...
                const modifyOutput = async(modify) => {
                    ...
                    const values = await custom.query(`
            ...
            const statsUser =  async() => {
                ...
                if (msg.filter(i => !i.startsWith('@')).join(' ').length<5) {
            ...
            const statsChannel = async() => {
                if (msg[1]) {
                    const checkChannels = await custom.query(`
                        SELECT *
            ...
            const statsBruh = async() => {
                ...
                const countResults = await custom.query(`
                    SELECT *
                    FROM bruh
                    WHERE channel=?`,
                    [channel.replace('#', '')]);
                ...
                if (!msg[1]) {
                    ...
                    if (channel === "#nymn") {
                        return `${user['username']}, total of ${countResults.length} racists
                    ...
                    if (!countResults.length) {
                        return `${user['username']}, no racists in this channel, we coo RlyTho Clap`;
                    }
                ...
                if (msg.includes('#all')) {
                    const countAllResults = await custom.query(`
                        SELECT COUNT(*) as count
                        FROM bruh`);

                    const channelCount = await  custom.query(`
                        SELECT COUNT(*) as count
                        FROM channels_logger`);

                    const rowsToday = await custom.query(`
                        SELECT COUNT(*) AS count
                        FROM bruh
                ...
                if (msg[1].toLowerCase() === 'apollo' && channel === "#nymn") {
                    const getCount = await custom.query(`
                        SELECT *
                        FROM stats
                        WHERE type="counter"`);
                ...
                if (typeof channelParse != "undefined") {
                    const checkChannel = await custom.query(`SHOW TABLES LIKE ?`, [`logs_${channelParse.replace('#', '')}`]);
                    ...
                    const channelData = await custom.query(`
                ...
                const msgParsed = msg[1].toLowerCase().replace('@', '').replace(',', '');
                ...
                const countResultsUser = countResults.filter(i => i.username === msgParsed);
            ...
            const statsLines = async() => {
                ...
                if (username) {
                    ...
                    if (channelName) {
                        ...
                        const checkChannel = await custom.query(`SHOW TABLES LIKE ?`, [`logs_${channelName.replace('#', '')}`]);
                        ...
                        const userLines = await custom.query(`
                            SELECT COUNT(username) as value
                            FROM ??
                            WHERE username=?`,
            ...
            const statsColor = async() => {
                ...
                if (!msg[1]) {
                    if (user['color'] === null) {
                        const usersData = await custom.query(`
                ...
                if (msg[1].startsWith('#')) {
                    ...
                    const checkHexFromMessage = await custom.query(`
                        SELECT *
                        FROM user_list
                        WHERE color=?`,
                        [msg[1]]);
                ...
                const checkUserFromMessage = await custom.query(`
                    SELECT *
                    FROM user_list
                    WHERE username=?`,
                    [msg[1].replace('@', '')]);
                ...
                const userColor = await custom.query(`
                    SELECT *
                    FROM user_list
                    WHERE username=?`,
                    [msg[1].replace('@', '')]);
