    ...
    invocation: async (channel, user, message) => {
        try {
            ...
            const statsBruh = async() => {
                ...
                const countResults = await custom.doQuery(`
                    SELECT *
                    FROM bruh
                ...
                if (msg.includes('#all')) {
                    const countAllResults = await custom.doQuery(`
                        SELECT COUNT(*) as count
                        FROM bruh
                        `);

                    const channelCount = await  custom.doQuery(`
                        SELECT COUNT(*) as count
                        FROM channels_logger
                        `);

                    const rowsToday = await custom.doQuery(`
                        SELECT COUNT(*) AS count
                        FROM bruh
            ...
            const statsColor = async() => {
                ...
                if (!msg[1]) {
                   ...
                   if (Number(usersData[0].users) === 0 || Number(usersData[0].users) === 1) {
                        ...
                        return `${user['username']}, you are the only one with color ${color.name.value} ${user['color']} in my database PogChamp !`;
                    }
                ...
                if (msg[1].startsWith('#')) {
                    ...
                    if (color.rgb.fraction.r === null) {
                        return `${user['username']}, this color Hex does not exist.`;
                    }

                    const checkHexFromMessage = await custom.doQuery(`
                        SELECT COUNT(*) AS count
                        FROM user_list
                        WHERE color="${msg[1]}"
                        `);
                    return `${user['username']}, there are ${checkHexFromMessage[0].count} users with color ${color.name.value} (${checkHexFromMessage[0].color})`;
                }

                const checkUserFromMessage = await custom.doQuery(`
                    SELECT *
                    FROM user_list
                ...
                await custom.doQuery(`
                    UPDATE user_list
                    SET color="${(updateColor.chatColor === null) ? 'gray' : updateColor.chatColor}"
                    WHERE username="${msg[1].replace('@', '')}"
                    `);

                const userColor = await custom.doQuery(`
                    SELECT *
                    FROM user_list
                    WHERE username="${msg[1].replace('@', '')}"
                    `);
                ...
                if (Number(usersData[0].users) === 1 || Number(usersData[0].users) === 0) {
                   ...
                   ${color.name.value} ${userColor[0].color}! PogChamp`;
                }
