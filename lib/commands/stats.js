...
module.exports = {
  ...
  name: 'kb stats',
  invocation: async (channel, user, message, platform, _, kb) => {
    try {
      ...
      const checkChannel = await utils.query('SHOW TABLES LIKE ?', [`logs_${this.channel}`]);
      ...
      const statsUser = async () => {
        ...
        if (channel !== '#nymn') {
          ...
          const randomRow = utils.randomElement(rows);
      ...
      const statsColor = async () => {
        ...
        if (!msg[1]) {
          ...
          const color = await got(
            `https://www.thecolorapi.com/id?hex=${user['color'].replace('#', '')}`
          ).json();
          ...
          return `${user['username']}, you share your color (${color.name.value} ${
            user['color']
        ...
        if (msg[1].startsWith('#')) {
          const color = await got(
            `https://www.thecolorapi.com/id?hex=${msg[1].replace('#', '')}`
          ).json();
          if (!color?.rgb ?? true) {
        ...
        const updateColor = await got(
          `https://api.ivr.fi/twitch/resolve/${msg[1].replace('@', '')}`
        ).json();
        ...
        const color = await got(
          `https://www.thecolorapi.com/id?hex=${userColor[0].color.replace('#', '')}`
        ).json();
        ...
        return `${user['username']}, this user's color (${color.name.value} ${
          userColor[0].color
        })
