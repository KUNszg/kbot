    ...
    try {
      ...
      const statsUser = async () => {
        ...
        if (channel != '#nymn') {
          ...
          const response = await Promise.all([utils.doQuery(mysql.format(sql, select)), utils.doQuery(mysql.format(sql2, select2))]);
      ...
      const statsMessage = async () => {
        ...
        const occurence = await Promise.all([utils.doQuery(mysql.format(sql, inserts)), utils.doQuery(mysql.format(sql2, inserts2))]);
