	...
	invocation: async (channel, user, message, args) => {
		try {
            ...
            const statsColor = async() => {
                ...
                if (!msg[1]) {
                    if (user['color'] === '') {
                        return `${user['username']}, you don't have any color set.`;
                    }
                    ...
                    const color = await got(`https://www.thecolorapi.com/id?hex=${user['color'].replace('#', '')}`);
                ...
                if (checkIfUserExists[0].color === "gray" || checkIfUserExists[0].color === null) {
                    return `${user['username']}, this user has not set a color (default).`;
                }
                ...
                if (userData[0].users === 1) {
                    return `${user['username']}, that user is the only one with color
                    ${checkIfUserExists[0].color}! PogChamp`;
                }

                const color = await got(`https://www.thecolorapi.com/id?hex=${checkIfUserExists[0].color.replace('#', '')}`);
                return `${user['username']}, this user's color (${color.name.value} ${checkIfUserExists[0].color}) is being used by
                ${userData[0].users-1} other users`;
            }
