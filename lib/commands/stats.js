			...
			if (((msg[0] != "-channel" && msg[0] != "-bruh") && msg.length != 0)) {	
				...
				if (!msg[1]) {
					...
					if (channel === '#haxk') {
						...
						if (channelValueCache[0].valueCount === 0) {
							return `${user['username']}, total of ${channelValueCache[0].valueCount} racists 
				...
				// kb stats -bruh [user]
					...
					kb.whisper(user['username'], `Please be patient, I'm fetching data from my database for command stats -bruh [user]`)
