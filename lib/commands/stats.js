			...
			if (((msg[0] != "-channel" && msg[0] != "-bruh") && msg.length != 0)) {	
				...
				// kb stats -bruh [user]
					...
					const userValue = await custom.doQuery(`
						SELECT COUNT(*) AS value 
						FROM logs_${channelParsed} 
						WHERE AND username="${msg[1]}" AND (message LIKE "%nigg%") 
						`);
					...
					kb.kb.whisper(user['username'], `Please be patient, I'm fetching data from my database for command stats -bruh [user]`)
