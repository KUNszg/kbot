	...
	invocation: async (channel, user, message, args) => {
		try {
            ...
            const statsColor = async() => {
                ...
                const updateColor = await got(`https://api.ivr.fi/twitch/resolve/${msg[1].replace('@', '')}`).join();
                await custom.doQuery(`
                    UPDATE user_list
                    SET color="${updateColor.chatColor}"
                    WHERE username="${msg[1].replace('@', '')}"
                    `);
