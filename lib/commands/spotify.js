...
module.exports = {
  ...
  invocation: async (channel, user, message) => {
    try {
      ...
      const msg = message.split(' ')[1] === 'spotify' ? utils.getParam(message.toLowerCase()) : utils.getParam(message.toLowerCase(), 1);
            ...
            try {
              const tokenSpotify = await got(
                `https://accounts.spotify.com/api/token?grant_type=refresh_token&refresh_token=${await refresh(
                  'refresh_token',
                  username
                )}&client_secret=${creds.client_secret_spotify}&client_id=${creds.client_id_spotify}`,
                {
                  method: 'POST',
                  headers: {
      ...
      if (msg[0]) {
        if (msg[0].startsWith('@')) {
          ...
          const response = `${user.username}, current song playing on ${findUser[0].username.replace(/^(.{2})/, '$1\u{E0000}')}'s Spotify:
                    ...
                    [${minutes(song.progress_ms)}:${seconds(song.progress_ms)}/${minutes(song.item.duration_ms)}:${seconds(
      ...
      if (msg[0] === 'top') {
        ...
        if (findUser) {
          ...
          const songs = assignNumbers(new utils.ModifyOutput(topTracksData.items.slice(0, 4).map(i => i.name)).trimmer());

          const artists = new utils.ModifyOutput(topTracksData.items.slice(0, 4).map(i => i.artists[0].name)).trimmer();
      ...
      if (msg[0] === 'search') {
        ...
        if (type === 'track') {
          ...
          try {
            ...
            if (err.error?.code === 403) {
              return (
                result + search.tracks.items[0].external_urls.spotify + ` - you can play it on your spotify by using kb yoink ${trackId}`
              );
            }
          }
        }
      ...
      const response =
        `Current song playing on ${user.username}'s Spotify: ${songTitle} by ${artist}, ${song.is_playing ? '▶ ' : '⏸ '}` +
