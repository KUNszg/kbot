    ...
    invocation: async (channel, user, message) => {
        try {
                ...
                trimmer() {
                    const noPing = (str) => {
                        if (str.toLowerCase() === "constera" || str.toLowerCase() === "nymn") {
                            return str.replace(/^(.{2})/, "$1\u{E0000}").replace("undefined", "no data");
                        }
                        return str.replace("undefined", "no data");
                    }
            ...
            if (msg[0] === "top") {
                ...
                if (findUser) {
                    ...
                    if (msg.find(i => i.toLowerCase() === "artists") || msg.find(i => i.toLowerCase() === "artist")) {
                        ...
                        let [artist1, artist2, artist3, artist4, artist5]  = new ModifyOutput(
                                topArtistData.items.slice(0, 5).map(i => i.name), 20
                            ).trimmer();

                        return `${user['username']}, that person's ${timeRange} term most listened to Spotify artists are:
                        (1) ${artist1}, (2) ${artist2}, (3) ${artist3}, (4) ${artist4}, (5) ${artist5}`;
                    }
                    ...
                    let [song1, song2, song3] = new ModifyOutput(
                            topTracksData.items.slice(0, 3).map(i => i.name)
                        ).trimmer();

                    let [artist1, artist2, artist3] = new ModifyOutput(
                            topTracksData.items.slice(0, 3).map(i => i.artists[0].name)
                        ).trimmer();

                    return `${user['username']}, that person's ${timeRange} term most listened to Spotify songs are:
                    (1) ${song1} by ${artist1}, (2) ${song2} by ${artist2}, (3) ${song3} by ${artist3}`;
            ...
            if (msg[0] === "volume" || msg[0] === "vol") {
                ...
                if (!msg[1]) {
                    await apiCall('https://api.spotify.com/v1/me/player/volume?volume_percent=50', user['username'], 'PUT');
                    if (custom.strictChannels(channel)) {
                ...
                await apiCall(`https://api.spotify.com/v1/me/player/volume?volume_percent=${msg[1].replace('%', '')}`, user['username'], 'PUT');
