            ...
            if (msg[0]) {
                if (msg[0].startsWith('@')) {
                    ...
                    try {
                        ...
                        if ((err.response?.status ?? true) && err.name === "HTTPError: Response code 401 (Unauthorized)") {
                            return `${user['username']}, 401 Error (unauthorized), my bot is not authorized to resolve this request, it's probaby your Spotify settings.`;
                        }
                        if (msg[0]) {
                            if (msg[0].startsWith('@')) {
