        ...
        try {
            ...
            if ((msg[0] === "play" || msg[0] === "queue") || (msg[0] === "p" || msg[0] === "q")) {
                try {
                    ...
                    return `${user['username']}, song ${search.tracks.items[0].name} by ${search.tracks.items[0].artists[0].name} has been added to your playback queue.`;
                } catch (err) {
                    return `${user['username']}, an error has occured FeelsDankMan`;
                    console.log(err)
                }
            }
