        ...
        try {
            ...
            if (msg[0] === "music" || msg[0] === "lastfm") {
                if (msg[1] === "register") {
                    ...
                    if (!checkIfUserExists?.error ?? true) {
                ...
                const lastfmCurrPlaying = await got(`http://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&user=${checkIfUserRegistered[0].access_token}&api_key=${creds.lastfmApiKey}&format=json`).join();
