        ...
        try {
            ...
            if ((msg[0] === "play" || msg[0] === "queue") || (msg[0] === "p" || msg[0] === "q")) {
                try {
                    ...
                    if (checkIfStartedPlaying.item.name != search.tracks.items[0].name) {
                        await apiCall('https://api.spotify.com/v1/me/player/next', 'POST');
                    }

                    return `${user['username']}, now playing: ${search.tracks.items[0].name} by
                    ${search.tracks.items[0].artists[0].name}`;
                } catch (err) {
                    console.log(err);
                    return `${user['username']}, an error has occured FeelsDankMan try to search with more specific query.`;
                }
            }
