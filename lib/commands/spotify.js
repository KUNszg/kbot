            ...
            if (msg[0] === "top") {
                ...
                if (findUser) {
                    ...
                    if (msg.find(i => i.match(/\bartist[s]?\b/i))) {
                        const topArtistData = await apiCall(`https://api.spotify.com/v1/me/top/artists?time_range=${timeRange.toLowerCase()}_term`, _user);

                        const artists = new ModifyOutput(topArtistData.items.slice(0, 5).map(i => i.name), 23).trimmer();
                    ...
                    // kb spotify top @user
                    const topTracksData = await apiCall(`https://api.spotify.com/v1/me/top/tracks?time_range=${timeRange.toLowerCase()}_term`, _user);

                    const songs = assignNumbers(
                        new ModifyOutput(topTracksData.items.slice(0, 4).map(i => i.name)).trimmer()
                        );

                    const artists = new ModifyOutput(topTracksData.items.slice(0, 4).map(i => i.artists[0].name)).trimmer();
