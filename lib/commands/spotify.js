...
module.exports = {
    ...
    description: `kb spotify [start/pause/volume/skip/shuffle] || if you want access to the command, ask KUNszg :)`,
    permission: 0,
    cooldown: 5000,
    invocation: async (channel, user, message, args) => {
        try {
            ...
            if (!permittedUsers.length) {
                if (channel === '#forsen') {
                    return `${user['username']}, currently this command is disabled for non-permitted users.`;
                }
                return `${user['username']}, currently this command is disabled for non-permitted users.`;
            }
            ...
            if (msg[0].replace('stop', 'pause') === "pause") {
                if (!premiumUsers.length) {
                    return `${user['username']}, only premium users can use this parameter :(`;
                }
            ...
            if (msg[0].replace("vol", "volume") === "volume") {
                if (!premiumUsers.length) {
                    return `${user['username']}, only premium users can use this parameter :(`;
                }
            ...
            if (msg[0].replace("random", "shuffle") === "shuffle") {
                if (!premiumUsers.length) {
                    return `${user['username']}, only premium users can use this parameter :(`;
                }

                if (msg[1].replace('on', 'true') === "true") {
                ...
                if (msg[1].replace('off', 'false') === "false") {
           ...
           if (err) {
                ...
                kb.whisper('kunszg', JSON.stringify(err))
