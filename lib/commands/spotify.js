        ...
        try {
            ...
            try {
                if (msg[0] === "music" || msg[0] === "lastfm") {
                    if (msg[1] === "unregister") {
                        const checkIfUserRegistered = await custom.query(`
                            SELECT *
                            FROM access_token
            ...
            const refresh = async(col, userRaw = user['user-id']) => {
                ...
                const refresh = await custom.query(`
                    SELECT *
                    FROM access_token
            ...
            const apiCall = async(link, userRaw, method = "GET") => {
                if (userRaw && userRaw != user['username']) {
                    const username = userRaw.replace("@", "").replace(",", "");
                    const checkIfRenewed = await custom.query(`
                        SELECT *
                        FROM access_token
            ...
            if (msg[0]) {
                if (msg[0].startsWith('@')) {
                   ...
                   if (checkIfOptedOut.length && (user['username'] != this.msg)) {
                    ...
                    const isRegistered = await custom.query(`
                        SELECT *
                        FROM access_token
                        WHERE platform="spotify" AND user=?`,
                        [findUser[0].userId]);
                    ...
                    try {
                        ...
                        const youtubeUser = await custom.youtube(`${song.item.name} by ${song.item.artists[0].name}`);
            ...
            const userList = await custom.query(`
                SELECT *
                FROM access_token
                WHERE platform="spotify"`);
            ...
            if (msg[0] === "top") {
                ...
                if (findUser) {
                    this.user = findUser.toLowerCase().replace(/@|,/g, "");
                   ...
                   if (checkIfOptedOut.length && (user['username'] != this.user)) {
                        return `${user['username']}, that user has opted out from being a target of this command.`;
                    }
                    ...
                    if (msg.find(i => i.toLowerCase() === "artists") || msg.find(i => i.toLowerCase() === "artist")) {
                        const topArtistData = await apiCall(`https://api.spotify.com/v1/me/top/artists?time_range=${timeRange.toLowerCase()}_term`, this.user);
                    ...
                    const topTracksData = await apiCall(`https://api.spotify.com/v1/me/top/tracks?time_range=${timeRange.toLowerCase()}_term`, this.user);
