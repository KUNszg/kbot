            ...
            */
            ...
            if (msg[0] === "yoink") {
                ...
                try {
                    ...
                    const checkIfStartedPlaying = async () => {
                        return await apiCall('https://api.spotify.com/v1/me/player/currently-playing', user['username'], 'GET');
                    }

                    const [playing, artist] = new ModifyOutput([yoinkedSong.item.name, yoinkedSong.item.artists[0].name]).trimmer();

                    if ((await checkIfStartedPlaying()).item.uri != yoinkedSong.item.uri) {
                        await apiCall('https://api.spotify.com/v1/me/player/next', user['username'], 'POST');

                        if ((await checkIfStartedPlaying()).item.uri != yoinkedSong.item.uri) {
                            return `${user['username']}, you have yoinked song ${playing} by ${artist} from
                            ${username.replace(/^(.{2})/, "$1\u{E0000}")}'s Spotify and it has been added to your playback queue TriHard 🎶`;
                        }
                    }
                    ...
                    if (msg.find(i => i === "-t")) {
                        await apiCall(`https://api.spotify.com/v1/me/player/seek?position_ms=${yoinkedSong.progress_ms}`, user['username'], 'PUT');
