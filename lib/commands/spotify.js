    ...
    invocation: async (channel, user, message, args) => {
        try {
            ...
            if (channel === "#forsen") {
                ...
                ${song.is_playing ? '▶ ' : '⏸ '} [${minutes(song.progress_ms)}:${seconds(song.progress_ms)}/${minutes(song.duration_ms)}:${seconds(song.duration_ms)}]
                link 👉 ${song.item.external_urls.spotify.replace('https://', '').replace(/\./g, '(dot)').replace('(dot)', '.')}`;
            }
            return `${user['username']}, current song playing on my spotify: ${song.item.name} by ${song.item.artists[0].name},
            ${song.is_playing ? '▶ ' : '⏸ '}
