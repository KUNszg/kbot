...
module.exports = {
    ...
    invocation: async (channel, user, message, args) => {
        try {
            ...
            if (Number(err.response.statusCode) === 401) {
                return `${user['username']}, error 401 (Unauthorized) - my bot is not authorized to resolve this request, it's probaby your Spotify settings FeelsDankMan`;
            }
            if (Number(err.response.statusCode) === 403) {
                return `${user['username']}, error 403 Forbidden - the server understood the request, but is refusing to fulfill it FeelsDankMan`;
            }
            if (Number(err.response.statusCode) === 404) {
                return `${user['username']}, error 404 Not Found - Spotify web service is most likely down FeelsDankMan`;
            }
            if (Number(err.response.statusCode) === 429) {
                return `${user['username']}, error 429 Too Many Requests - my bot is sending too many queries at once, rate limiting has been applied FeelsDankMan`;
            }
            if (Number(err.response.statusCode) === 500) {
                return `${user['username']}, error 500 - Internal Server Error FeelsDankMan`;
            }
            if (Number(err.response.statusCode) === 502) {
                return `${user['username']}, error 502 Bad Gateway - the server was acting as a gateway or proxy and received an invalid response from the upstream server FeelsDankMan`;
            }
            if (Number(err.response.statusCode) === 503) {
                return `${user['username']}, error 503 Service Unavailable - the server is currently unable to handle the request due to a temporary condition which will be alleviated after some delay FeelsDankMan`;
            }
            console.log(err);
            return `${user['username']}, no song is currently playing on your spotify FeelsDankMan`;
        }
