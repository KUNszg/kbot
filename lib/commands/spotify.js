    ...
    invocation: async (channel, user, message) => {
        try {
            ...
            class ModifyOutput {
                ...
                trimmer() {
                    if (this.input.toLowerCase() === "constera" || this.input.toLowerCase() === "nymn") {
                        return this.input.replace(/^(.{2})/, "$1\u{E0000}");
                    }
            ...
            const apiCall = async(link, method = "GET", username) => {
                if (username) {
            ...
            if (msg[0] === "top") {
                ...
                if (findUser) {
                    const checkIfUserRegistered = await custom.doQuery(`
                        SELECT *
                        FROM access_token
                        WHERE username="${findUser.replace("@", "").replace(",", "").toLowerCase()}"
                        `);
                    ...
                    if (msg[1] != "artists") {
                        const topTracksData = await apiCall("https://api.spotify.com/v1/me/top/tracks?time_range=long_term");

                        const song1 = new ModifyOutput(topTracksData.items[0].name);
                        const artist1 = new ModifyOutput(topTracksData.items[0].artists[0].name);

                        const song2 = new ModifyOutput(topTracksData.items[1].name);
                        const artist2 = new ModifyOutput(topTracksData.items[1].artists[0].name);

                        const song3 = new ModifyOutput(topTracksData.items[2].name);
                        const artist3 = new ModifyOutput(topTracksData.items[2].artists[0].name);

                        return `${user['username']}, your most listened to Spotify songs are:
                        (1) ${song1.trimmer()} by ${artist1.trimmer()},
                        (2) ${song2.trimmer()} by ${artist2.trimmer()},
                        (3) ${song3.trimmer()} by ${artist3.trimmer()}`;
                    }

                    const topArtistData = await apiCall("https://api.spotify.com/v1/me/top/artists?time_range=long_term");

                    const artist1 = new ModifyOutput(topArtistData.items[0].name);
                    const artist2 = new ModifyOutput(topArtistData.items[1].name);
                    const artist3 = new ModifyOutput(topArtistData.items[2].name);

                    return `${user['username']}, your most listened to Spotify artists are:
                    (1) ${artist1.trimmer()},
                    (2) ${artist2.trimmer()},
                    (3) ${artist3.trimmer()}`;
                }
