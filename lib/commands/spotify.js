...
module.exports = {
    ...
    description: `song that currently plays on kunszg's spotify -- cooldown 5s`,
    permission: 0,
    cooldown: 5000,
    invocation: async (channel, user, message, args) => {
        try {
            ...
            if (channel === "forsen") {
                return `${user['username']}, current song playing on kunszg's spotify: ${song.item.name} by ${song.item.artists[0].name},
