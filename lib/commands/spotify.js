            ...
            if ((msg[0] === "play" || msg[0] === "queue") || (msg[0] === "p" || msg[0] === "q")) {
                try {
                    ...
                    if ((msg[0] === "p" || msg[0] === "play") && (checkIfStartedPlaying.item.name != search.tracks.items[0].name)) {
                        ...
                        return `${user['username']}, now playing: ${playing.trimmer()} by ${artist.trimmer()}`;
                    }

                    return `${user['username']}, ${playing.trimmer()} by ${artist.trimmer()} has been added to playback queue.`;
                } catch (err) {
                    console.log(err);
