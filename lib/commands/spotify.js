    ...
    invocation: async (channel, user, message) => {
        try {
            ...
            */
            ...
            if (msg[0] === "account" || msg[0] === "acc") {
                const accountData = await  apiCall("https://api.spotify.com/v1/me", user.username, "GET");

                kb.whisper(user.username, `Your registered username is ${account.display_name}
                with plan "${account.product}", ${account.followers.total} followers
                ${account.href} (this data can be accessed only by you)`);

                return `${user.username}, I whispered you your registered account details.`;
            }
