...
module.exports = {
	...
	invocation: async (channel, user, message, platform) => {
		try {
			const msg = (message.split(' ')[1].toLowerCase() === "removed") || (message.split(' ')[1].toLowerCase() === "emote") ? utils.getParam(message, 1) : utils.getParam(message);
            ...
            if ((msg[0] === "list" || msg[0] === "link") || (msg[0] === "-list" || msg[0] === "-link")) {
                ...
                if (channel === "#forsen") {
                    return `${user['username']}, see full list of emotes: kunszg(dot)com/emotes?search=${channel.replace('#', '')}`;
                }
            ...
            if (msg[0] === "api") {
                ...
                if (channel === "#forsen") {
                    return `${user['username']}, //FFZ api(DOT)frankerfacez(DOT)com/v1/room/id/${userId[0].userId} //BTTV api(DOT)betterttv(DOT)net/3/cached/users/twitch/${userId[0].userId}`;
                }
            ...
            if ((msg?.[0] ?? false) && (msg[0] === "emote")) {
                ...
                const emotesCurr = await utils.query(`
                    ...
                    FROM emotes
                    WHERE url IS NOT NULL AND emote=?`, [this.userMsg]);
                ...
                const emoteSets = emotesRem.concat(emotesCurr);
                if (!emoteSets.length) {
                    return `${user['username']}, no emotes were found.`;
                }

                function emote(index) {
                    return (emoteSets[index].type === "bttv") ?
                    emoteSets[index].url.replace('https://cdn.betterttv.net/emote/', 'https://betterttv.com/emotes/').replace('/1x', '') :
                    `https://www.frankerfacez.com/emoticon/${emoteSets[index].emoteId}-${emoteSets[index].emote}`;
                }

                if (msg.join(' ').match(/i:\[?(\d+)\]?/)) {
                    this.index = msg.join(' ').match(/i:\[?(\d+)\]?/)[0].replace(/i:|\[|\]/g, '');

                    if (typeof emoteSets[Number(this.index)] === "undefined") {
                        return `${user['username']}, this index does not exist.`;
                    }

                    if (!emoteSets[Number(this.index)]?.url ?? true) {
                        return `${user['username']}, this emote does not have url saved.`;
                    }
                ...
                if (emoteSets.length > 1) {
                    return `${user['username']}, ${emoteSets.length} results found, use i:[number] as parameter to get them. i:0 : ${emote(0)}`;
                }

                return `${user['username']}, ${emoteSets[0].url.replace('https:', '')}`;
            }
