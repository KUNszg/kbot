                ...
                try {
                    ...
                    const checkForRepeatedEmotesSeventv = async (emote, id, type) => {
                        ...
                        if (!updateEmotes) {
                            ...
                            await utils.query(`
                                INSERT INTO emotes (channel, emote, url, type, sevenTvId, date)
                                VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`,
                                [channelName, emote, emoteLink, type, id]);
                        }
                    }
                    ...
                    for (let j = 0; j < emotes.length; j++) {
                        ...
                        if (emotes[j].type === "7tv") {
                            if (!seventvEmotes.some(i => emotes[j].sevenTvId === i.id)) {
                                ...
                                await utils.query(`
                                    INSERT INTO emotes_removed (channel, emote, url, sevenTvId, date, type)
                                    VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP, "7tv")`,
                                    [channelName, emotes[j].emote, emotes[j].url, emotes[j].sevenTvId]);
                            }
                        }
                    }
