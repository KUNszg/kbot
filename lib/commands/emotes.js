		...
		try {
            ...
            if ((msg?.[0] ?? false) && (msg[0] === "removed")) {
                ...
                if (userSpecifiedChannel.length) {
                    ...
                    const checkChannel = await custom.query("SHOW TABLES LIKE ?", [`logs_${userSpecifiedChannel[0].toLowerCase().replace('#', '')}`]);

                    if (!checkChannel.length) {
                        return `${user['username']}, I'm not logging the channel you specified :/`;
                    }

                    const latestRemovedEmotes = await custom.query(`
                        SELECT *
                        FROM emotes_removed
                        WHERE channel=?
                        ORDER BY date
            ...
            if (msg[0]) {
                // check if channel exists in the database
                const checkChannel = await custom.query("SHOW TABLES LIKE ?", [`logs_${msg[0].toLowerCase().replace('#', '')}`]);

                if (!checkChannel.length) {
                    return `${user['username']}, I'm not logging the channel you specified :/`;
                }

                const latestEmotes = await custom.query(`
                    SELECT *
                    FROM emotes
                    WHERE channel=?
                    ORDER BY date
