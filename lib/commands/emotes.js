	...
	invocation: async (channel, user, message, platform) => {
		try {
            ...
            if ((msg?.[0] ?? false) && (this.message === "emote")) {
                ...
                const emotesBttvApi = await got(encodeURI(`https://api.betterttv.net/3/emotes/shared/search?query=${this.userMsg}&limit=100`)).json();

                this.bttvEmoteSet = emotesBttvApi.map(i => `https://betterttv.com/emotes/${i.id}`);

                const emoteSets = emotesCurr.concat(emotesRem);

                this.ffzEmoteSet = emoteSets.map(i => `https://www.frankerfacez.com/emoticon/${i.emoteId}-${i.emote}`)
                ...
                const emote = (index) => {
                    if (this.platform) {
                        if (this.platform.toLowerCase().replace("-", "") === "bttv") {
                            this.results = this.bttvEmoteSet.length;
                            return this.bttvEmoteSet[index];
                        }
                        this.results = this.ffzEmoteSet.length;
                        return this.ffzEmoteSet[index];
                    }
                    this.results = this.emoteSets.length;
                ...
                if (msg.join(' ').match(/i:\[?(\d+)\]?/)) {
                    ...
                    if (!this.emoteSets[Number(this.index)]?.url ?? true) {
                ...
                if (this.emoteSets.length > 1) {
                    return `${user['username']}, ${this.results} results found, use i:[number] as parameter to get them. i:0 : ${emote(0)}`;
                }
