...
module.exports = {
	...
	invocation: async (channel, user, message, platform) => {
		try {
			...
			let msg = (this.message=== "removed") || (this.message === "emote") ? utils.getParam(message, 1) : utils.getParam(message);
			...
			if (msg[0] === "yoink" || msg[0] === "reload") {
                ...
                } catch (err) { return `${user['username']}, no BTTV emotes were found.` }
                ...
                } catch (err) { return `${user['username']}, no FFZ emotes were found.`}
            ...
            const findParam = msg.filter(i => i.match(/\b-?(bttv|ffz)\b/gi)).map(i => i.toLowerCase());

            msg = msg.filter(i => !i.match(/\b-?(bttv|ffz)\b/gi));
			...
			return `${user['username']}, recently added emotes ${type} in this channel: ${emotes.join(', ')}`
		} catch (err) {
			utils.errorLog(err)
