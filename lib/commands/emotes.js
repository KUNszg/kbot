	...
	invocation: async (channel, user, message, platform) => {
		try {
            ...
            if (msg[0] === "api") {
                ...
                if (channel === "#forsen") {
                    return `${user['username']}, //FFZ api(DOT)frankerfacez(DOT)com/v1/room/${channel.replace('#', '')} //BTTV api(DOT)betterttv(DOT)net/3/cached/users/twitch/${userId[0].userId}`;
                }
                return `${user['username']}, //FFZ https://api.frankerfacez.com/v1/room/${channel.replace('#', '')} //BTTV https://api.betterttv.net/3/cached/users/twitch/${userId[0].userId}`;
            }
