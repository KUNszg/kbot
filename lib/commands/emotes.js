	...
	invocation: async (channel, user, message, platform) => {
		try {
            ...
            if ((msg?.[0] ?? false) && (msg[0] === "emote")) {
                ...
                if (msg.match(/i:\[?(\d+)\]?/)) {
                    this.index = msg.match(/i:\[?(\d+)\]?/)[0].replace(/i:|\[|\]/g, '');
                    ...
                    return `${user['username']}, i:${this.index} : https:${emoteSets[Number(this.index)].url.replace('https:', '')} `;
                }

                if (!emoteSets[0]?.url ?? true) {
                    return `${user['username']}, this emote does not have url saved.`;
                }


                if (emoteSets.length > 1) {
                    return `${user['username']}, ${emoteSets.length} results found, use i:[number] in command to get them. i:0 : https:${emoteSets[0].url.replace('https:', '')} `;
                }
