  ...
  invocation: async (channel, user, message, platform) => {
    try {
      ...
      if (msg[0] === 'yoink' || msg[0] === 'reload') {
        ...
        // BTTV
        try {
          const bttvEmotes = await got(`${api[1]}/cached/users/twitch/${userId[0].userId}`).json();
        ...
        if (!cache.filter(i => i[userMsg]).length) {
          ...
          for (let limit = 100, offset = 0; offset < 1500; ) {
            const list = await got(
              encodeURI(`https://api.betterttv.net/3/emotes/shared/search?query=${userMsg}&offset=${offset}&limit=${limit}`)
            ).json();
          ...
          let ffzEmoteSet = emoteSets.filter(i => i.url != null).map(i => `https://www.frankerfacez.com/emoticon/${i.emoteId}-${i.emote}`);
