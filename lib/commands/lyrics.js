...
module.exports = {
    ...
    invocation: async (channel, user, message, platform, discord) => {
        try {
            ...
            const getLyrics = got(`${api}?title=${encodeURIComponent(msg)}`).json();
            ...
            fs.writeFileSync(`${dir}/lyrics_${randString}.txt`, `${getLyrics.title} by ${getLyrics.author} \n\n ${getLyrics.lyrics}`);

            discord.channel.send(":)", { attachment: [`${dir}/lyrics_${randString}.txt`], name: `${getLyrics.title} by ${getLyrics.author}` })
