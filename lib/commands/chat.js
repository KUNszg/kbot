		...
		try {
			...
			if (user['username'] === "kunszg") {
				...
				async function second() {
					...
					const secondLoop = await fetch(`${creds.chat}${await cache[0].split(' ').join("+").normalize("NFD").replace(/[\u0300-\u036f]/g, "")}`)
						.then(response => response.json());
					console.log(await secondLoop)
					cache.length = 0;
					await kb.kb.say('kunszg', await secondLoop.response.toString());
					await cache.push(await secondLoop.response.replace(/\*/g, '').replace(/./g, '').replace(/-/g, '').replace(/\?/g, '').replace(/!/g, ''));

					const firstLoop = await fetch(`${creds.chat}${cache[0].split(' ').join("+").normalize("NFD").replace(/[\u0300-\u036f]/g, "")}`)
						.then(response => response.json());
					console.log(await secondLoop)
					cache.length = 0;
					await kb.kb.say('kunszg', await firstLoop.response.toString());
					await cache.push(await firstLoop.response.replace(/\*/g, '').replace(/./g, '').replace(/-/g, '').replace(/\?/g, '').replace(/!/g, ''));
				}
