		...
		try {
			...
			if (user['username'] === "kunszg") {
				...
				async function second() {
					...
					const secondLoop = await fetch(`${creds.chat}${await cache[0].split(' ').join("+").normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\*/g, '').replace(/./g, '').replace(/-/g, '').replace(/\?/g, '').replace(/!/g, '')}`)
						.then(response => response.json());
					cache.length = 0;
					kb.kb.say('kunszg', await secondLoop.response);
					cache.push(await secondLoop.response);

					const firstLoop = await fetch(`${creds.chat}${cache[0].split(' ').join("+").normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\*/g, '').replace(/./g, '').replace(/-/g, '').replace(/\?/g, '').replace(/!/g, '')}`)
						.then(response => response.json());
					cache.length = 0;
