...
module.exports = {
	...
	invocation: async (channel, user, message, platform) => {
		try {
			...
			const checkIfExists = await custom.query(`
				SELECT *
				FROM user_list
				WHERE username=?;`,
                [msg[0]]);
			...
			const checkTrustedList = await custom.query(`
				SELECT *
				FROM trusted_users
				WHERE username=?`,
                [msg[0]]);
			...
			switch (msg[1]) {
				...
				case 'status':
					...
					(${checkTrustedList[0].status}), added: ${custom.humanizeDuration((Date.now() - Date.parse(checkTrustedList[0].added))/1000)} ago`;
                case '-1':
                case 'terraria':
                    if (!checkTrustedList.length) {
                        await custom.query(`
