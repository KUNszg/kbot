	...
	invocation: async (channel, user, message, args) => {
		try {
			...
			switch (msg[1]) {
				...
				case 'check':
				case 'status':
					return `${user['username']}, permissions of that user: ${checkTrustedList[0].permissions} (${checkTrustedList[0].status}), added: ${new Date(checkTrustedList[0].added).toISOString()}`;
                ...
                case 'terraria':
                    if (checkTrustedList.length === 0) {
                        await custom.doQuery(`
                            ...
                            VALUES ("${msg[0]}", "-1:terraria", "active", CURRENT_TIMESTAMP)
                            `);
                    }
                    await custom.doQuery(`
                        UPDATE trusted_users
                        SET permissions="-1:terraria"
                        WHERE username="${msg[0]}"
                        `);
