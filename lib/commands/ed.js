...
module.exports = {
	...
	unregister - unregister from the database | silence - mute the feedback | force - force your reminder trigger`,
	permission: 0,
	cooldown: 10000,
			...
			switch (msg[0]) {
				...
				case 'force':
					...
					bot.kb.whisper(user['username'], `I will remind you to enter dungeon in
						${custom.format(edApi.next_entry.toFixed(0) - (Date.now(new Date())/1000))} (forced reminder)`);
					break;

				case 'register':

					if (!resultsRegister.length) {
