  ...
  invocation: async (channel, user, message, platform) => {
    try {
      ...
      if (msg[0] === 'backup') {
        ...
        backupStart = (Date.now() - new Date(backupStart[backupStart.length - 1].date).getTime()) / 1000;

        let backupEnd = lastBackup.filter(i => i.type === 'end');
        backupEnd = (Date.now() - new Date(backupEnd[backupEnd.length - 1].date).getTime()) / 1000;

        return `${user.username}, last backup of the database and bot files started ${utils.humanizeDuration(
          backupStart
