...
module.exports = {
    ...
    invocation: async (channel, user, message, alias) => {
        ...
        const msg = ((_message === "lastfm" || _message === "music") || _message === "song") ?
        ...
        try {
            if (msg[0]) {
                if (msg[0].startsWith("@")) {
                    ...
                    const lastfmCurrPlaying = await got(`${api}/?method=user.getrecenttracks&user=${userData[0].access_token}&api_key=${creds.lastfmApiKey}&format=json`).json();
