    ...
    invocation: async (channel, user, message) => {
        try {
           ...
           const suggestionID = (await utils.query(`
            ...
            return `${user['username']}, suggestion saved with ID ${suggestionID} PogChamp - ⚠ channel join requests might take extra time
            (3-5 days) to process since Im on vacation and away from PC until late August. Thanks for your patience :)`;
        } catch (err) {
            utils.errorLog(err)
