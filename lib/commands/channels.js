...
module.exports = {
  ...
  invocation: async (channel, user, message) => {
    try {
      ...
      const dbSize = (Number(shell.execSync('sudo du -s /var/lib/mysql/kbot').toString().split('\t')[0]) / 1024 / 1024).toFixed(2);
      ...
      if (msg[0] === 'rename') {
        ...
        for (let i = 0; i < _user.length - 1; i++) {
          ...
          if (checkTables.length) {
            ...
            kb.say(
              channel,
              `${user.username}, logs_${oldName} was found` + ` as a original logs table for ${newName}, starting the rename process...`
            );
            ...
            setTimeout(() => {
              kb.say(channel, `${user.username}, channel owner logs have been updated to the new name.`);
              kb.say(newName, `Successfully finished the channel after-rename process, you will be able to use the bot in ~30s :)`);
            }, 7000);
