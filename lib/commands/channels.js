...
module.exports = {
    ...
    invocation: async (channel, user, message) => {
        try {
            ...
            if (msg.find(i => i.match(/^(-)?\btime\b/g))) {
                if (msg[1]) {
                    channel = msg[1]
                }
                ...
                const getChannel = getBotChannels.find(i => i.channel === channel);

                if (!getChannel) {
                    return `${user['username']}, channel was not recognized.`;
                }

                const stats = await got("https://kunszg.com/api/stats").json();

                const getChannelRowCount = await utils.query(`
                    SELECT count(*) AS count
                    FROM channels
                    WHERE ID <= ?`, [getChannel.ID]);

                if (!stats.bot.isRestarting) {
                    return `${user['username']}, bot is currently not restarting, but if it were,
                    it would take around ${utils.humanizeDuration(getChannelRowCount[0].count * 2)} to join this channel.`;
                }

                const fs = require('fs');

                const uptime = Math.trunc(Number(fs.readFileSync("../../data/temp_api_uptime.txt").toString()));

                console.log(uptime)

                return `${user['username']}, my bot will join this channel in ${utils.humanizeDuration(uptime - (getChannelRowCount[0].count * 2))}`;
            }
