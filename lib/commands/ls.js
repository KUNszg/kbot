    ...
    invocation: async (channel, user, message) => {
        try {
           ...
           if (checkIfOptedOut.length && (user['username'] != msg[0])) {
            ...
            if (lastSeenChannel === "haxk" || lastSeenChannel === "axo__") {
                return `${user['username']}, this user was last seen in a channel that is opted out ${custom.humanizeDuration(time)} ago`;
            }

            return `${user['username']}, this user's last message was in channel ${lastSeenChannel.replace(/^(.{2})/, "$1\u{E0000}")} ${custom.humanizeDuration(time)} ago and their message was: ${result.trimmer()}`;
        } catch (err) {
            custom.errorLog(err)
            return ``;
        }
