    ...
    invocation: async (channel, user, message) => {
        try {
           ...
           if (checkIfOptedOut.length && (user['username'] != msg[0])) {
            ...
            return `${user['username']}, this user's last message was in channel ${lastSeenChannel.replace(/^(.{2})/, "$1\u{E0000}")} ${utils.humanizeDuration(time)} ago and their message was: ${result.trimmer()}`;
        } catch (err) {
            utils.errorLog(err)
            return ``;
        }
