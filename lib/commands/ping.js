...
module.exports = {
	...
	invocation: async (channel, user, message) => {
		try {
			...
			return `${user['username']}, ${ping(message)} FeelsDankMan 🏓 ppHop 🏓💻 live status: https://kunszg.com
            running for ${utils.humanizeDuration(process.uptime())} (master, ${commitData[0].sha}, commit ${commitCount})`;
		} catch (err) {
			utils.errorLog(err)
