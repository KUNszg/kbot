				...
				if (await custom.banphrasePass(randomLine[0].message, channel.replace('#', '')).banned === true) {
					if (channel==="#nymn" || channel==="#forsen") {
						if (timeDifference>48) {
							kb.whisper(user['username'], getChannel.replace(/^(.{2})/, "$1\u{E0000}") + ' (' + (timeDifference/24).toFixed(0) + 'd' + modifyOutput(1000));
						} else {
							kb.whisper(user['username'], getChannel.replace(/^(.{2})/, "$1\u{E0000}") + ' (' + custom.formatUptime(timeDifferenceRaw/1000) + modifyOutput(1000));
						}
			...
			if (await custom.banphrasePass(randomLine[0].message, channel.replace('#', '')).banned === true) {
				if (channel==="#nymn" || channel==="#forsen") {
					if (timeDifference>48) {
						...
						kb.whisper(user['username'], '(' + custom.formatUptime(timeDifferenceRaw/1000) + modifyOutput(1000));
					}
					return `${user['username']}, result is banphrased, I whispered it to you tho cmonBruh`;
				}
