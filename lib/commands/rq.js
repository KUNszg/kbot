	...
	invocation: async (channel, user, message, args) => {
		try {
				...
				if (await custom.banphrasePass(randomLine[0].message, channel.replace('#', '')).banned === true) {
					if (channel==="#nymn" || channel==="#forsen") {
						if (timeDifference>48) {
							kb.whisper(user['username'], `${getChannel.replace(/^(.{2})/, "$1\u{E0000}")} (${(timeDifference/24).toFixed(0)}d ago) ${randomLine[0].username}: ${randomLine[0].message}`);
						} else {
							kb.whisper(user['username'], `${getChannel.replace(/^(.{2})/, "$1\u{E0000}")} (${custom.formatUptime(timeDifferenceRaw/1000)} ago) ${randomLine[0].username}: ${randomLine[0].message}`);
						}
					...
					return getChannel.replace(/^(.{2})/, "$1\u{E0000}") + ' (' + custom.formatUptime(timeDifferenceRaw/1000) + modifyOutput(100);
				}
			...
			if (await custom.banphrasePass(randomLine[0].message, channel.replace('#', '')).banned === true) {
				if (channel==="#nymn" || channel==="#forsen") {
					if (timeDifference>48) {
						kb.whisper(user['username'], `(${(timeDifference/24).toFixed(0)}d ago) ${randomLine[0].username}: ${randomLine[0].message.substr(0, modify)}`);
					} else {
						kb.whisper(user['username'], `(${custom.formatUptime(timeDifferenceRaw/1000)}) ago) ${randomLine[0].username}: ${randomLine[0].message.substr(0, modify)}`);
					}
					return `${user['username']}, result is banphrased, I whispered it to you tho cmonBruh`;
				}
