		...
		try {
			...
			switch (msg[0]) {
				...
				case "ping":
					...
					for (let i=0; i<=14; i++) {
						if (i === 5) {
							return `${partyDataPing[0].username.replace(/^(.{2})/, "$1\u{E0000}")} is binging, it is time FeelsDankMan 🔔 | ${arrayOfUsers.slice(0, 4).join(', ')}`;
						} 
						if (i === 10) {
							return `${partyDataPing[0].username.replace(/^(.{2})/, "$1\u{E0000}")} is binging, it is time FeelsDankMan 🔔 | ${arrayOfUsers.slice(4, 9).join(', ')}`;
						} 
						if (i === 15) {
							return `${partyDataPing[0].username.replace(/^(.{2})/, "$1\u{E0000}")} is binging, it is time FeelsDankMan 🔔 | ${arrayOfUsers.slice(9, 14).join(', ')}`;
						} 
					}
					return '';
