		...
		try {
			...
			switch (msg[0]) {
				...
				case "ping":
					...
					if (usersInPartyPing.length > 10) {
						return `${user['username']}, I cannot bing more than 15 people monkaS you can do it yourself by getting a list of users with "kb party list"`;
					}

					const arrayOfUsers = usersInPartyPing.map(i => i.username != partyDataPing[0].username);
					for (let i=0; i<=15; i++) {
						if (i === 5) {
							return `${partyDataPing[0].username} is binging, it is time FeelsDankMan 🔔 | ${arrayOfUsers.slice(0, 5).join(', ')}`;
						} 
						if (i === 10) {
							return `${partyDataPing[0].username} is binging, it is time FeelsDankMan 🔔 | ${arrayOfUsers.slice(5, 10).join(', ')}`;
						} 
						if (i === 15) {
							return `${partyDataPing[0].username} is binging, it is time FeelsDankMan 🔔 | ${arrayOfUsers.slice(10, 15).join(', ')}`;
						} 
					}
					return '';
