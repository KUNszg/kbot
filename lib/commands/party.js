...
module.exports = {
	...
	Useful for checking who wants to play cs or who is ready for something eShrug`,
	permission: 0,
	cooldown: 5000,
				...
				case "j":
					...
					if (partyDataByName.length === 0) {
						...
						await custom.doQuery(`
							...
							VALUES ("${partyDataByID[0].ID}", "${user['username']}", "${user['user-id']}", "${partyDataByID[0].party_name}", "N", "N", "N", "10", "inactive", "${now.addTime(129600).toISOString().slice(0, 19).replace('T', ' ')}", CURRENT_TIMESTAMP)
							`);
					...
					await custom.doQuery(`
						INSERT INTO party (ID, username, userid, party_name, creator, editor, ready, slots, status, expires, date)
						VALUES ("${partyDataByName[0].ID}", "${user['username']}", "${user['user-id']}", "${partyDataByName[0].party_name}", "N", "N", "N", "10", "inactive", "${now.addTime(129600).toISOString().slice(0, 19).replace('T', ' ')}",CURRENT_TIMESTAMP)
						`);
