			...
			switch (msg[0]) {
				...
				case "j":
					...
					const partyDataByID = await custom.doQuery(`
						SELECT *
						FROM party
						WHERE ID="${msg[1]}" AND status="active"
						`); 
					...
					if (partyDataByName.length === 0) {
						...
						await custom.doQuery(`
							...
							`);
						return `${user['username']}, successfully joined party ${partyDataByID[0].name} owned by ${partyDataByID[0].userid}`;
					}
					...
					await custom.doQuery(`
						...
						`);
					return `${user['username']}, successfully joined party ${partyDataByName[0].name} owned by ${partyDataByName[0].userid}`;
				...
				case "ditch":
					...
					await custom.doQuery(`
						DELETE FROM party
						WHERE userid="${user['username']}" AND ID="${userPartyData[0].ID}"
						`);
				...
				case "go":
					...
					const usersReady = await custom.doQuery(`
						SELECT *
						FROM party
						WHERE ID="${userPartyData[0].ID}" AND ready="Y"
						`);
					const usersInParty = await custom.doQuery(`
						SELECT *
						FROM party
						WHERE ID="${userPartyData[0].ID}"
						`);

					if (userPartyData[0].ready === "Y") {
						return `${user['username']}, you are already ready with ${usersReady.length}/${usersInParty.length} 
						other users in party ${userPartyData[0].party_name} (ID ${userPartyData[0].ID})`;
					}
					...
					await custom.doQuery(`
						...
						`);
					return `${user['username']} is ready! ${usersReady.length}/${usersInParty.length} users are ready 
					in party ${userPartyData[0].party_name}`;
				...
				case "status":
					...
					if (partyDataName.length === 0) {
						...
						name: ${partyDataID[0].name.substr(0, 20)}, creator: ${partyDataID[0].username}, 
					...
					name: ${partyDataName[0].name.substr(0, 20)}, creator: ${partyDataName[0].username}, 
