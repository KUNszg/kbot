		...
		try {
			...
			switch (msg[0]) {
				...
				case "c":
					...
					await custom.doQuery(`
						UPDATE party
						SET expires="${now.addTime(43200).toISOString().slice(0, 19).replace('T', ' ')}"
						WHERE ID="${partyData[0].ID}"
						`);
					console.log(now.addTime(43200).toISOString().slice(0, 19).replace('T', ' '))

					return `${user['username']}, your party "${partyData[0].party_name}" (ID ${partyData[0].ID}) 
					has been successfully created with 10 default slots, it will auto-disband in 12h, check "kb help party" 
					for more options.`;
