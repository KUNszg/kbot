		...
		try {
			...
			if (typeof findFlag != 'undefined') {
				...
				if (!findMessage.join(' ').includes('to')) {
					return `${user['username']}, wrong syntax, use the command like that: "kb locate -dist helsinki to munich"`;
				}

				if (findparams[0].toLowerCase() === findparams[1].toLowerCase()) {
					return `${user['username']}, please provide different locations to search for :)`;
				}

				const findparams = msg.shift(1).join(' ').split('to');
				const distance = await got(encodeURI(`https://www.dystans.org/route.json?stops=${findparams[0]}|${findparams[1]}`)).json();

				if (Number(distance.distance) === 0) {
					return `${user['username']}, One of the places doesn't exist or one of them is closer than 1km to the other.`;
				}

				return `${user['username']}, distance between ${findMessage[0].normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()} and 
				${findMessage[1].normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()} is ${distance.distance}km`;
			}
			...
			if (!custom.hasNumber(msg[0])) {
				const locationNames = await got(`${creds.geonames}${encodeURI(msg.join(' '))}&maxRows=1&username=kunszg`).json()
