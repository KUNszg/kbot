  ...
  invocation: async (channel, user, message) => {
    try {
      ...
      if (flag) {
        ...
        const distance = await got(encodeURI(`https://www.distance24.org/route.json?stops=${param1}|${param2}`)).json();
      ...
      if (!utils.hasNumber(msg[0])) {
        const locationNames = await got(
          `http://api.geonames.org/searchJSON?q=${encodeURI(msg.join(' '))}&maxRows=1&username=kunszg`
        ).json();
        ...
        const population = !Number(location.population) ? '' : ` population: ${location.population} | `;
        const info = !Number(location.population) && location.fcodeName === 'populated place' ? 'no further data' : location.fcodeName;

        return `${user['username']}, results: ${locationNames.totalResultsCount} |
                location: ${country}${region}${city} | ${population} | info: ${info}`.replace(/ń/g, 'n');
      }
