	...
	invocation: async (channel, user, message, args) => {
		try {
			...
			if (typeof findFlag != 'undefined') {
				...
				return `${user['username']}, distance between ${findparams[0].normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()} and 
				${findparams[1].normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()} is ${distance.distance}km`;
			}
