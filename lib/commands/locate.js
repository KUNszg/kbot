	...
	invocation: async (channel, user, message, args) => {
		try {
			...
			if (typeof findFlag != 'undefined') {
				...
				const distance = await got(encodeURI(`https://www.dystans.org/route.json?stops=${findparams[0].split(' ').slice(0, -1).join(' ')}|${findparams[1].split(' ').splice(1).join(' ')}`)).json();
				...
				const param1 = findparams[1].split(' ').splice(1).join(' ').normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
				const param2 = findparams[0].split(' ').slice(0, -1).join(' ').normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();

				return `${user['username']}, distance between ${param1} and ${param2} is ${distance.distance}km`;
			}
