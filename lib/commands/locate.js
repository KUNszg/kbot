	...
	invocation: async (channel, user, message) => {
		try {
			...
			if (!custom.hasNumber(msg[0])) {
				...
				const region = !location.adminName1.length ? ' ' : `, ${location.adminName1.replace("ń", "n")}`;
				...
				const info = (!Number(location.population) && location.fcodeName === "populated place") ?
			...
			custom.errorLog(err)
			return `${user['username']}, ${err.message.replace(/(?:https?|ftp):\/\/[\n\S]+/g, '')} FeelsDankMan !!!`;
		}
	}
}