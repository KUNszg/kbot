	...
	invocation: async (channel, user, message, args) => {
		try {
			...
			if (typeof findFlag != 'undefined') {
				...
				const param1 = findparams[0].split(' ').slice(0, -1).join(' ').normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
				const param2 = findparams[1].split(' ').splice(1).join(' ').normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
