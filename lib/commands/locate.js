	...
	invocation: async (channel, user, message, args) => {
		try {
			...
			if (typeof findFlag != 'undefined') {
				...
				const distance = await got(encodeURI(`https://www.dystans.org/route.json?stops=${findMessage[0]}|${findMessage[1]}`)).json();

				if (findMessage[0].toLowerCase() === findMessage[1].toLowerCase()) {
