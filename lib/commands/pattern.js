...
module.exports = {
  ...
  invocation: async (channel, user, message, platform) => {
    try {
      ...
      const patterns = [{ pattern: 'pyramid' }, { pattern: 'square' }, { pattern: 'inverse' }, { pattern: 'triangle' }];
      const patternChosen = patterns.filter(i => i.pattern === msg[0]);
      ...
      if (patternChosen[0].pattern === 'pyramid') {
        const createPyramid = height => {
          for (var i = 1; i <= height; i++) {
            ...
            for (var j = 1; j <= i; j++) row += ' ' + emote[Math.floor(Math.random() * emote.length)];
            kb.say(channel, row);
          }
          for (var i = height - 1; i > 0; i--) {
            var row = '';

            for (var j = i; j > 0; j--) row += ' ' + emote[Math.floor(Math.random() * emote.length)];
            kb.say(channel, row);
          }
        };
