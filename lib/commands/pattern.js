...
module.exports = {
	...
	invocation: async (channel, user, message, args) => {
		try {
			...
			const emote = message.replace(/[\u{E0000}|\u{206d}]/gu, "").split(" ").splice(4);
			const msgP = message.replace(/[\u{E0000}|\u{206d}]/gu, "").split(" ").splice(3);
			const emoteP = message.replace(/[\u{E0000}|\u{206d}]/gu, "").split(" ").splice(4);
			...
			if (user['user-id'] != '178087241') {
				if (msg[1]>10 && Number(await custom.checkPermissions(user['username']))>=3) {
					return `${user['username']}, I can't allow pyramids higher than 10 for your permissions 😬`;
				}

				if (msg[1]>5 && await custom.checkPermissions(user['username'])==2) {
