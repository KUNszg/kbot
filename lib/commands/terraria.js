...
module.exports = {
    ...
    aliases: null,
    description: `commands fetching data from my Terraria server. Whisper/ping KUNszg for server IP. Parameters: https://pastebin.com/UxNvgiWi`,
    description_formatted: `commands fetching data from my Terraria server. Whisper/ping KUNszg for server IP. Parameters: pastebin (dot) com/UxNvgiWi`,
    permission: -1,
    cooldown: 8000,
    invocation: async (channel, user, message, args) => {
        try {
            ...
            async function endpoint(input) {
                if (input.includes('?')) {
                    const terraria = await got(`${creds.terrariaIP}${input}&token=${creds.terraria}`).json();
                    return terraria;
                }
                const terraria = await got(`${creds.terrariaIP}${input}?token=${creds.terraria}`).json();
                return terraria;
            ...
            if (!msg[0]) {
                return `${user['username']}, no parameter provided, see "kb help terraria"`;
            }
            ...
            switch (msg[0]) {
                ...
                case 'broadcast':
                    if (await custom.checkPermissions(user['username'])==-1 || await custom.checkPermissions(user['username'])>3) {
                        ...
                        return `${user['username']}, message has been sent.`;
                    }
                    break;
