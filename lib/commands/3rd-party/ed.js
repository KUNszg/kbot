...
module.exports = {
  invocation: async (channel, user, message) => {
    try {
      ...
      if (getEdData.next_entry.toFixed(0) > Date.now() / 1000 && getEdData.next_entry.toFixed(0) - Date.now() / 1000 < 3565) {
        kb.whisper(
          user['username'],
          `Your dungeon entry is still on cooldown
                        (${utils.humanizeDuration(getEdData.next_entry.toFixed(0) - Date.now() / 1000)})
