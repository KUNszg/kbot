...
module.exports = {
    invocation: async (channel, user, message) => {
        try{
            ...
            if (Math.floor(getEdData.next_entry) > (Date.now()/1000) &&
                (Math.floor(getEdData.next_entry) - (Date.now()/1000))<3565) {
                    kb.whisper(user['username'], `Your dungeon entry is still on cooldown
                        (${utils.humanizeDuration(Math.floor(getEdData.next_entry - (Date.now()/1000)))})
