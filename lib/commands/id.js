    ...
    try {
      ...
      if (!msg[0]) {
        ...
        return (
          ...
          `${getSenderData[0].firstSeen === 'haxk' ? '[EXPUNGED]' : getSenderData[0].firstSeen.replace(/^(.{2})/, '$1\u{E0000}')}.`
        );
      }
      ...
      return (
        `${user['username']}, ${getUserData[0].username.replace(/^(.{2})/, '$1\u{E0000}')}'s ` +
        ...
        `in channel ${getUserData[0].firstSeen === 'haxk' ? '[EXPUNGED]' : getUserData[0].firstSeen.replace(/^(.{2})/, '$1\u{E0000}')}.`
      );
    } catch (err) {
      utils.errorLog(err);
