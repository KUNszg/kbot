		...
		try {
			...
			if (!msg[0]) {
				...
				if (dateToSec>259200) {
					...
					in channel ${getSenderData[0].channel_first_appeared.replace(/^(.{2})/, "$1\u{E0000}")}.`;
				}

				// if user was seen less than 4 days ago
				return `${user['username']}, Your internal user ID is ${getSenderData[0].ID},
				you were first seen by the bot ${custom.formatUptime(dateToSec)} ago 
				in channel ${getSenderData[0].channel_first_appeared.replace(/^(.{2})/, "$1\u{E0000}")}.`;
			}
			...
			if (dateToSec2>259200) {
				...
				in channel ${getUserData[0].channel_first_appeared.replace(/^(.{2})/, "$1\u{E0000}")}.`;
			}

			// if user was seen less than 4 days ago
			return `${user['username']}, user ${getUserData[0].username.replace(/^(.{2})/, "$1\u{E0000}")} 
			has internal ID ${getUserData[0].ID} and was first seen by the bot ${custom.formatUptime(dateToSec2)} ago
			in channel ${getUserData[0].channel_first_appeared.replace(/^(.{2})/, "$1\u{E0000}")}.`;
