	...
	invocation: async (channel, user, message) => {
		try {
			...
			if (!msg[0]) {
				...
				const getSenderData = await custom.query(`
					SELECT *
                    FROM user_list
					WHERE userId=?`,
                    [user['user-id']]);
			...
			ID in my user list is ${getUserData[0].ID} and their Twitch ID is ${getUserData[0].userId} they were first seen by the bot ${custom.humanizeDuration(dateToSec)} ago
