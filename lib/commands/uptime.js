...
module.exports = {
    ...
    invocation: async (channel, user, message) => {
        try {
            const loggedMemoryUsage = await custom.query(`SELECT * FROM memory`);
            ...
            return `${user['username']}, code is running for ${custom.humanizeDuration(codeUptime)} and has ${lines} lines,
            memory usage: ${memoryUsage} MB, host is up for ${custom.humanizeDuration(serverUptime)} FeelsDankMan`;
        } catch (err) {
            custom.errorLog(err)
