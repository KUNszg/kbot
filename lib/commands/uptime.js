...
module.exports = {
    ...
    invocation: async (channel, user, message) => {
        try {
            ...
            return user['username'] + ", code has" + lines + " lines, latest commit: "
                + utils.humanizeDuration(diff/1000) + " ago, memory usage: " + this.memoryUsage +
                " MB, server is up for " + utils.humanizeDuration(serverUptime) + " FeelsDankMan";
        } catch (err) {
            utils.errorLog(err)
