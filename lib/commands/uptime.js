...
const redis = require('../utils/connection.js').Redis;
...
module.exports = {
    ...
    invocation: async (channel, user, message) => {
        try {
            ...
            return `${user['username']}, code has${lines} lines, latest commit: ${utils.humanizeDuration(diff/1000)} ago,
            memory usage: ${this.memoryUsage} MB, Redis keys: ${await redis.size()}, host is up for ${utils.humanizeDuration(serverUptime)} FeelsDankMan`;
        } catch (err) {
            utils.errorLog(err)
