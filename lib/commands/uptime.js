...
module.exports = {
    ...
    invocation: async (channel, user, message, args) => {
        try {
            ...
            return `${user['username']}, code is running for ${custom.secondsToDhms(codeUptime)}, has ${lines} lines total,
            memory usage: ${used} MB, host is up for ${custom.secondsToDhms(serverUptime)}h FeelsDankMan`;
        } catch (err) {
            custom.errorLog(err)
