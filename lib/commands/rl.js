		...
		try {
            ...
            if (msg[0] === "rq") {
                ...
                if (userSpecifiedChannel) {
                    ...
                    return `${userSpecifiedChannel.replace(/^(.{2})/, "$1\u{E0000}")} (${custom.secondsToDhm(timeDifference)} ago) ${randomLine[0].username}: ${result.trimmer()}`;
                }
                ...
                return `(${custom.secondsToDhm(timeDifference)} ago) ${randomLine[0].username}:  ${result.trimmer()}`;
            }
			...
			if ((msg?.[0] ?? false) && msg[0] != "") {
                ...
                // kb rl [#channel]
				if ((global?.userSpecifiedChannel ?? false) && (!global?.userSpecifiedUsername ?? true)) {
                ...
                // kb rl [@user] [#channel]
                if ((global?.userSpecifiedChannel ?? false) && (global?.userSpecifiedUsername ?? false)) {
                    if (userSpecifiedChannel === '#supinic' || userSpecifiedChannel === "#haxk") {
                        return `${user['username']}, specified channel is opted out from being a target of this command flag.`;
                    }
                ...
                // kb rl [user]
                if (!global?.userSpecifiedChannel ?? true) {
                    // check if user exists in the database
                    ...
                    const timeDifference = (Math.abs(Date.now() - (Date.parse(randomLine.date)))/1000);
