		...
		try {
			...
			if (typeof msg[0] !== 'undefined' && msg[0] != '') {
				...
				if (typeof getChannel != 'undefined') {
					...
					return (timeDifference>172800) ? 
					`${getChannel} (${custom.secondsToDhm(timeDifference)} ago) ${modifyOutput(100)}` : 
					`${getChannel} (${custom.format(timeDifference)} ago) ${modifyOutput(100)}`;
				}
				...
				const randomLine = await custom.doQuery(`
					...
					INNER JOIN
					    (SELECT ROUND(
					       RAND() *
					      (SELECT MAX(ID) FROM logs_${channel.replace('#', '')} ) * (RAND() * 10)) AS id
					     ) AS x
					WHERE
