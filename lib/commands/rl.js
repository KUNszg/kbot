	...
	invocation: async (channel, user, message, platform) => {
		try {
			...
			const msg = (message.split(' ')[1] === "rq") ?
                utils.getParam(message, 1) : utils.getParam(message);
            ...
            if (msg[0] === "rq") {
                ...
                if (userSpecifiedChannel) {
                    ...
                    const checkInputChannel = await utils.query(
                        "SHOW TABLES LIKE ?",
                    ...
                    if (channel === "#nymn") {
                        randomResults = await utils.query(`
                            ...
                            WHERE t.id >= x.id and username=? LIMIT 30;`,
                    ...
                    return `${userSpecifiedChannel.replace(/^(.{2})/, "$1\u{E0000}")} (${utils.humanizeDuration(timeDifference)} ago) ${randomResult.username}: ${result.trimmer()}`;
                }
                ...
                const timeDifference = (Math.abs(Date.now() - (Date.parse(randomResults[0].date)))/1000);

                const result = new ModifyOutput(randomResults[0].message);

                return `(${utils.humanizeDuration(timeDifference)} ago) ${randomResults[0].username}:  ${result.trimmer()}`;
            }
			...
			if (!msg[0]) {
                ...
                return `(${utils.humanizeDuration(timeDifference)} ago) ${randomLine[0].username}:  ${result.trimmer()}`;
			}
			...
			if ((msg?.[0] ?? false) && msg[0] != "") {
				...
				if (typeof userSpecifiedChannel != "undefined" && typeof userSpecifiedUsername === "undefined") {
                    ...
                    return `${userSpecifiedChannel.replace(/^(.{2})/, "$1\u{E0000}")} (${utils.humanizeDuration(timeDifference)} ago) ${randomLine[0].username}: ${result.trimmer()}`;
				}
                ...
                if (typeof userSpecifiedChannel != "undefined" && typeof userSpecifiedUsername != "undefined") {
                    ...
                    if (checkIfOptedOut.length && (user['username'] != userSpecifiedUsername.toLowerCase().replace(/@|,/g, ''))) {
                ...
                if (channel === "#forsen") {
                    return `${user['username']}, this channel is opted-out of this command`;
                }
                ...
                if (channel === "#nymn") {
                    const randomResults = await utils.query(`
                        SELECT t.*
                        FROM ?? AS t
                        JOIN (
                            SELECT ROUND(
                ...
                return `(${utils.humanizeDuration(timeDifference)} ago) ${randomResult.username}:  ${result.trimmer()}`;
			}
		} catch (err) {
			utils.errorLog(err)
			return ``;
