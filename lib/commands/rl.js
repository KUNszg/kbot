		...
		try {
			...
			if (!msg[0]) {
                ...
                if (channel === "#forsen") {
                    ...
                    `(${custom.secondsToDhm(timeDifference)} ago) ${modifyOutput(150)}` :
                    `(${custom.format(timeDifference)} ago) ${modifyOutput(150)}`;
                }
			...
			if (typeof msg[0] !== 'undefined' && msg[0] != '') {
				...
				if (typeof getChannel != 'undefined') {
                    ...
                    if (channel === "#forsen") {
                        return (timeDifference>172800) ?
                        `${getChannel} (${custom.secondsToDhm(timeDifference)} ago) ${modifyOutput(150)}` :
                        `${getChannel} (${custom.format(timeDifference)} ago) ${modifyOutput(150)}`;
                    }
