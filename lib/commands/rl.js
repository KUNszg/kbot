		...
		try {
            ...
            if (msg[0] === "rq") {
                ...
                if (userSpecifiedChannel) {
                    ...
                    const checkInputChannel = await custom.query(
                        "SHOW TABLES LIKE ?",
                    ...
                    this.channel = userSpecifiedChannel.replace('#', '');

                    const randomResults = await custom.query(`
                            SELECT t.*
                            FROM ?? AS t
                            JOIN (
                                SELECT ROUND(
                    ...
                    return `${userSpecifiedChannel.replace(/^(.{2})/, "$1\u{E0000}")} (${custom.humanizeDuration(timeDifference)} ago) ${randomResults[0].username}: ${result.trimmer()}`;
                }
                ...
                return `(${custom.humanizeDuration(timeDifference)} ago) ${randomResults[0].username}:  ${result.trimmer()}`;
            }

            // kb rl
			if (!msg[0]) {
				const maxID = await custom.query(`
				...
				const randomLine = await custom.query(`
			...
			if ((msg?.[0] ?? false) && msg[0] != "") {
				...
				if (typeof userSpecifiedChannel != "undefined" && typeof userSpecifiedUsername === "undefined") {
					...
					const checkIfLogging = await custom.query(`
						SHOW TABLES LIKE ?`,
                        [`logs_${userSpecifiedChannel.replace('#', '')}`]);
