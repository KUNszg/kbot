	...
	invocation: async (channel, user, message, args) => {
		try {
			...
			if (!msg[0]) {
				...
				const randomLine[0] = await custom.doQuery(`SELECT ID, username, message, date FROM 
				...
				function modifyOutput(modify) {
					if (!modify) {
						return ` ago) ${randomLine[0][0].username.replace(/^(.{2})/, "$1\u{E0000}")}: 
						${randomLine[0][0].message.substr(0, 350)}`;
					} else {
						return ` ago) ${randomLine[0][0].username.replace(/^(.{2})/, "$1\u{E0000}")}: 
						${randomLine[0][0].message.substr(0, modify)}`;
					}
				}

				const timeDifference = (Math.abs(serverDate - 
					(new Date(randomLine[0][0].date).getTime())))/1000/3600;
				const timeDifferenceRaw = (Math.abs(serverDate - (new Date(randomLine[0][0].date).getTime())));

				// check for banphrases...
				if (await custom.banphrasePass(randomLine[0][0].message).banned === true) {
					if (channel==="#nymn") {
