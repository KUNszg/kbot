			...
			if (!msg[0]) {
				...
				if (typeof getChannel != 'undefined') {
					if (getChannel === '#supinic' || getChannel === "#haxk") {
						return `${user['username']}, specified channel is opted out from being a targed of this command flag.`;
					}
					...
					if (await custom.banphrasePass(randomLine[0].message, channel.replace('#', '')).banned === true) {
						if (channel==="#nymn" || channel === "#forsen") {
							if (timeDifference>48) {
								...
								kb.whisper(user['username'], getChannel.replace(/^(.{2})/, "$1\u{E0000}") + ' (' + custom.formatUptime(timeDifferenceRaw/1000) + 
									modifyOutput());
							}
							return user['username'] + ', result is banphrased, I whispered it to you tho cmonBruh';
						}
						...
						// other channels
						if (timeDifference>48) {
							return getChannel.replace(/^(.{2})/, "$1\u{E0000}") + ' (' + (timeDifference/24).toFixed(0) + 'd' + modifyOutput();
						}
						return getChannel.replace(/^(.{2})/, "$1\u{E0000}") + ' (' + custom.formatUptime(timeDifferenceRaw/1000) + modifyOutput();
					}

					// check for channels
					if (channel === "#nymn") {
						if (timeDifference>48) {
							return getChannel.replace(/^(.{2})/, "$1\u{E0000}") + ' (' + (timeDifference/24).toFixed(0) + 'd' + modifyOutput(130);	
						}
						return getChannel.replace(/^(.{2})/, "$1\u{E0000}") + ' (' + custom.formatUptime(timeDifferenceRaw/1000) + modifyOutput(130);
					}
