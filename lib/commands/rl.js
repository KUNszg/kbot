...
module.exports = {
  ...
  invocation: async (channel, user, message, platform) => {
    try {
      ...
      const msg = message.split(' ')[1] === 'rq' ? utils.getParam(message, 1) : utils.getParam(message);

      if (msg.includes('teodorv')) {
        return 'FeelsBadMan <3';
      }

      const whitelist = ['logs_forsen', 'logs_nymn', 'logs_vadikus007', 'logs_kian', 'logs_pajlada', 'logs_fabzeef', 'logs_redshell'];
      ...
      if (msg[0] === 'rq') {
        ...
        if (userSpecifiedChannel) {
          ...
          const checkInputChannel = await utils.query('SHOW TABLES LIKE ?', [`logs_${userSpecifiedChannel.replace('#', '')}`]);
          ...
          return `${userSpecifiedChannel.replace(/^(.{2})/, '$1\u{E0000}')} (${utils.humanizeDuration(timeDifference)} ago) ${
      ...
      if ((msg?.[0] ?? false) && msg[0] != '') {
        ...
        if (typeof userSpecifiedChannel != 'undefined' && typeof userSpecifiedUsername === 'undefined') {
          ...
          const maxID = await utils.query(`SELECT MAX(ID) AS number FROM ??`, [`logs_${userSpecifiedChannel.replace('#', '')}`]);
