		...
		try {
			...
			if (typeof msg[0] !== 'undefined' && msg[0] != '') {
				...
				if (typeof getChannel != 'undefined' && typeof getUser === 'undefined') {
					...
					const timeDiff = (Math.abs(Date.now() - (Date.parse(randomLine[0].date)))/1000);

					getChannel = getChannel.replace(/^(.{2})/, "$1\u{E0000}");

                    if (channel === "#forsen") {
                        return (timeDiff>172800) ?
                        `${getChannel} (${custom.secondsToDhm(timeDiff)} ago) ${modifyOutput(93).length>93 ? modifyOutput(93) + '(...)' : modifyOutput(93)}` :
                        `${getChannel} (${custom.format(timeDiff)} ago) ${modifyOutput(93).length>93 ? modifyOutput(93) + '(...)' : modifyOutput(93)}`;
                    }

					return (timeDiff>172800) ?
					`${getChannel} (${custom.secondsToDhm(timeDiff)} ago) ${modifyOutput(440)}` :
					`${getChannel} (${custom.format(timeDiff)} ago) ${modifyOutput(440)}`;
				}
                ...
                if (typeof getChannel === 'undefined' && typeof getUser != 'undefined') {
                    ...
                    if (channel === "#forsen") {
                        return (timeDiff>172800) ?
                        ` (${custom.secondsToDhm(timeDiff)} ago) ${modifyOutput(93).length>93 ? modifyOutput(93) + '(...)' : modifyOutput(93)}` :
                        ` (${custom.format(timeDiff)} ago) ${modifyOutput(93).length>93 ? modifyOutput(93) + '(...)' : modifyOutput(93)}`;
                    }

    				return (timeDiff>172800) ?
    				` (${custom.secondsToDhm(timeDiff)} ago) ${modifyOutput(440)}` :
    				` (${custom.format(timeDiff)} ago) ${modifyOutput(440)}`;
                }
